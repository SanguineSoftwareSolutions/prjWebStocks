	...
	public clsPMSPaymentHdModel funPreparePaymentModel(clsPMSPaymentBean objPaymentBean, String clientCode,HttpServletRequest request, String userCode)
	{
		...
		if(objPaymentBean.getStrAgainst().equals("Reservation"))
		{
			String sql= "select ifnull(b.strCheckInNo,''),ifnull(b.strRegistrationNo,''),a.strReservationNo"
				+ " ,ifnull(b.strFolioNo,'') "
				+ " from tblreservationhd a left outer join tblfoliohd b on a.strReservationNo=b.strReservationNo "
				+ " where a.strReservationNo='"+objPaymentBean.getStrDocNo()+"' ";
		...
		{
			String sql= " select b.strCheckInNo,a.strRegistrationNo,b.strReservationNo,b.strFolioNo "
				+ " from tblcheckinhd a,tblfoliohd b "
				+ " where a.strCheckInNo=b.strCheckInNo and a.strCheckInNo='"+objPaymentBean.getStrDocNo()+"' ";
		    List list=objGlobalFunctionsService.funGetListModuleWise(sql, "sql");
		    if(list.size()>0)
			{
		    	for(int cnt=0;cnt<list.size();cnt++)
		...
		else if(objPaymentBean.getStrAgainst().equals("Folio-No"))
		{
			String sql= " select b.strCheckInNo,a.strRegistrationNo,b.strReservationNo,b.strFolioNo "
				+ " from tblcheckinhd a,tblfoliohd b "
				+ " where a.strCheckInNo=b.strCheckInNo and b.strFolioNo='"+objPaymentBean.getStrDocNo()+"' ";
		    List list=objGlobalFunctionsService.funGetListModuleWise(sql, "sql");
		    if(list.size()>0)
			{
		    	for(int cnt=0;cnt<list.size();cnt++)
		...
		{
			String sql= " select a.strCheckInNo,a.strRegistrationNo,a.strReservationNo,a.strFolioNo "
				+ " from tblbillhd a "
				+ " where a.strBillNo='"+objPaymentBean.getStrDocNo()+"'";
		    List list=objGlobalFunctionsService.funGetListModuleWise(sql, "sql");
		    if(list.size()>0)
			{
		    	for(int cnt=0;cnt<list.size();cnt++)
