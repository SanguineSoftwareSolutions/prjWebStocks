...
public class clsCheckOutDaoImpl implements clsCheckOutDao
{
	...
	/*try
	{
	    ...
	    long nextBillNo = 0;		
	    // generate nextBillNo No.
            nextBillNo = objGlobalFunctionsService.funGetNextNo("tblbillhd", "CheckOut", "strBillNo", clientCode);
	    String billNo = "BN" + String.format("%06d", nextBillNo);	
	    
	    String sqlSelectFolioHd="select '"+billNo+"','"+objGlobal.funGetCurrentDateTime("yyyy-MM-dd")+"',a.strFolioNo,a.strRegistrationNo,a.strReservationNo,'0.00','"+userCode+"','"+userCode+"' "
            +",'"+objGlobal.funGetCurrentDateTime("yyyy-MM-dd")+"','"+objGlobal.funGetCurrentDateTime("yyyy-MM-dd")+"','"+clientCode+"' "
            +"from tblfoliohd a "
            +"where a.strRoomNo='"+roomNo+"' and a.strClientCode='"+clientCode+"' ";
	    
	    String sqlInsertIntoBillHd="insert into tblbillhd ("+sqlSelectFolioHd+")";
	    
	    Query insertQuery=webPMSSessionFactory.getCurrentSession().createSQLQuery(sqlInsertIntoBillHd);
	    insertQuery.executeUpdate();
	    
	    Query deleteQuery=webPMSSessionFactory.getCurrentSession().createSQLQuery("delete from tblfoliohd where strRoomNo='"+roomNo+"' and strClientCode='"+clientCode+"' ");
    ...
    {	
		...
		String sql="update tblroom set strStatus='Free' "
			+ " where strRoomCode='"+objBillHdModel.getStrRoomNo()+"' and strClientCode='"+objBillHdModel.getStrClientCode()+"'";
