...
try
{
	 String clientCode = req.getSession().getAttribute("clientCode").toString();
		...
		// get all parameters
	    String sqlParametersCheckOutList = " SELECT bh.strReservationNo, IFNULL(h.strBookingTypeDesc,'NA'), "
	    		...
	    		+ " CONCAT(j.strFirstName,' ',j.strMiddleName,' ',j.strLastName),j.strGuestCode,bh.strBillNo,Sum(bd.dblDebitAmt) "
	    		+ " FROM tblbillhd bh "
	    		+ " LEFT OUTER JOIN tblbilldtl bd ON bh.strBillNo = bd.strBillNo AND bd.strClientCode='"+clientCode+"' "
	    		+ " LEFT OUTER JOIN tblcheckinhd ch ON ch.strCheckInNo = bh.strCheckInNo AND ch.strClientCode='"+clientCode+"' "
	    		+ " LEFT OUTER JOIN tblreservationhd a ON a.strReservationNo = bh.strReservationNo and a.strClientCode='"+clientCode+"' "
	    		...
	    		+ " WHERE DATE(bh.dteBillDate) "
	    		+ "  BETWEEN '"+fromDate+"' and '"+toDate+"' "
	    		+ " AND ch.strClientCode='"+clientCode+"' AND a.strPropertyCode='"+propertyCode+"' group by bh.strReservationNo ";  
		...
		checkOutListBean.setDblCreditAmt((arr[11] == null) ? 0 : Double.parseDouble(arr[11].toString()) );
		 ...
		 String sqlCheckOutListDtl = " select a.strFirstName,a.strMiddleName,a.strLastName,b.strRoomTypeDesc,a.strAddress,"
		 		...
		 		+ " from tblguestmaster a,tblroomtypemaster b,tblbillhd c,tblroom d,tblcheckindtl e, tblbilldtl f "
		 		+ " where  date(c.dteBillDate) between '"+fromDate+"' and '"+toDate+"' "
		 		+ " and f.strBillNo='"+strBillNo+"' "
