		    ...
		    public ModelAndView funSplitBillDo(@ModelAttribute("command") @Valid clsBillHdModel objBean, BindingResult result, HttpServletRequest req)
		    {
				...
				if (!result.hasErrors())
				{
					 	...
					 	if(!objBean.getStrBillNo().contains("-"))
					 	{
							...
							//    Set<clsBillDtlModel> tempDtl= new TreeSet<clsBillDtlModel>();
					 			if(splitType.equalsIgnoreCase("FOOD"))
					 			{
					 			//	Set<clsBillDtlModel> listDtl= objBillHdModel.getSetBillDtlModels();
//					 						objBillService.funLoadBillDtl(objBean.getStrBillNo(), clientCode);
					 			//	List<clsBillDtlModel> listDtl= objBillHdModel.getListBillDtlModels();
					 			...
					 			if(splitType.equalsIgnoreCase("LIQUOR"))
					 			{
...
//					 						objBillService.funLoadBillDtl(objBean.getStrBillNo(), clientCode);
					 			...
					 			if(splitType.equalsIgnoreCase("ROOM"))
					 			{
					 			//	Set<clsBillDtlModel> listDtl= objBillHdModel.getSetBillDtlModels();
//					 						objBillService.funLoadBillDtl(objBean.getStrBillNo(), clientCode);
					 			//	List<clsBillDtlModel> tempDtl= new ArrayList<clsBillDtlModel>();
...
//				 						objBillService.funLoadBillDtl(objBean.getStrBillNo(), clientCode);
				...
				req.getSession().setAttribute("successMessage", "Splited Bill No. : ".concat(splitBillCodes));
