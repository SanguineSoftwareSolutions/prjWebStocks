...
// Save folio posting
    ...
    public void funGenerateFolioPrintingReport(@RequestParam("fromDate") String fromDate, @RequestParam("toDate") String toDate, @RequestParam("folioNo") String folioNo, HttpServletRequest req, HttpServletResponse resp)
    {
		try
		{
		    ...
		    String sqlParametersFromFolio = "SELECT a.strFolioNo,e.strRoomDesc,a.strRegistrationNo,a.strReservationNo "
			    	+ " ,date(b.dteArrivalDate),b.tmeArrivalTime ,ifnull(date(b.dteDepartureDate),'NA'),ifnull(b.tmeDepartureTime,'NA')"
			    	+ " ,d.strGuestPrefix,d.strFirstName,d.strMiddleName,d.strLastName ,b.intNoOfAdults,b.intNoOfChild,'NA' "
			    	+ " FROM tblfoliohd a LEFT OUTER JOIN tblreservationhd b ON a.strReservationNo=b.strReservationNo "
			    	+ " LEFT OUTER JOIN tblguestmaster d ON a.strGuestCode=d.strGuestCode "
			    	+ " LEFT OUTER JOIN tblroom e ON a.strRoomNo=e.strRoomCode "
		    ...
		    if(folioDtl.size() > 0)
		    {
		    	...
		    	{
		    		sqlParametersFromFolio = "SELECT a.strFolioNo,e.strRoomDesc,a.strRegistrationNo,a.strReservationNo "
					    	+ " ,date(b.dteWalkinDate),b.tmeWalkinTime ,ifnull(date(b.dteCheckOutDate),'NA'),ifnull(b.tmeCheckOutTime,'NA')"
					    	+ " ,d.strGuestPrefix,d.strFirstName,d.strMiddleName,d.strLastName ,b.intNoOfAdults,b.intNoOfChild,'NA' "
					    	+ " FROM tblfoliohd a LEFT OUTER JOIN tblwalkinhd b ON a.strWalkinNo=b.strWalkinNo "
					    	+ " LEFT OUTER JOIN tblguestmaster d ON a.strGuestCode=d.strGuestCode "
					    	+ " LEFT OUTER JOIN tblroom e ON a.strRoomNo=e.strRoomCode "
					    	+ " where a.strFolioNo='" + folioNo + "' and a.strClientCode='"+clientCode+"'";
		    	}
	    ...
	    // get all parameters
			...
			// get folio details
				String sqlFolioDtl = "SELECT b.dteDocDate,b.strDocNo,b.strPerticulars,b.dblDebitAmt,b.dblCreditAmt,b.dblBalanceAmt "
				...
				for (int i = 0; i < folioDtlList.size(); i++)
				{
				    ...
				    else
				    {
						...
						sqlFolioDtl = "SELECT a.dteDocDate,a.strDocNo,b.strTaxDesc,b.dblTaxAmt,0,0 "
							+ " FROM tblfoliodtl a,tblfoliotaxdtl b where a.strDocNo=b.strDocNo "
							+ " and DATE(a.dteDocDate) BETWEEN '" + fromDate + "' AND '" + toDate + "' "
							+ " AND a.strFolioNo='" + folioNo + "' and a.strDocNo='"+docNo+"'";
			...
			// get payment details
				String sqlPaymentDtl = "SELECT b.dteDocDate,c.strReceiptNo,e.strSettlementDesc,'0.00' as debitAmt,d.dblSettlementAmt as creditAmt"
					+ " ,'0.00' as balance "
					+ " FROM tblfoliohd a LEFT OUTER JOIN tblfoliodtl b ON a.strFolioNo=b.strFolioNo "
					...
					+ " WHERE DATE(b.dteDocDate) BETWEEN '" + fromDate + "' AND '" + toDate + "' AND a.strFolioNo='" + folioNo + "' "
