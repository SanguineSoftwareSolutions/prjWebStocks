...
try
{
	 String clientCode = req.getSession().getAttribute("clientCode").toString();
		...
		// get all parameters
	    String sqlParametersExpArrival = "SELECT a.strReservationNo,ifnull(h.strBookingTypeDesc,'NA'), DATE_FORMAT(a.dteDateCreated,'%d-%m-%Y'),ifnull(c.strCorporateDesc,'NA'), " 
			+"ifnull(d.strDescription,'NA'), IFNULL(i.dblReceiptAmt,0), ifnull(e.strBookerName,'NA'),DATE_FORMAT(a.dteConfirmDate,'%d-%m-%Y'), "
			+"DATE_FORMAT(a.dteCancelDate,'%d-%m-%Y'),ifnull(f.strDescription,'NA'),ifnull(g.strBillingInstDesc,'NA'),concat(j.strFirstName,' ',j.strMiddleName,' ',j.strLastName) "
			+",j.strGuestCode FROM tblreservationhd a left outer join tblreservationdtl b on a.strReservationNo=b.strReservationNo  "
			+"left outer join tblcorporatemaster c on a.strCorporateCode=c.strCorporateCode  "
			+"left outer join tblagentmaster d on a.strAgentCode=d.strAgentCode  "
			+"left outer join tblbookermaster e on a.strBookerCode=e.strBookerCode  "
			+"left outer join tblbusinesssource f on a.strBusinessSourceCode=f.strBusinessSourceCode " 
			+"left outer join tblbillinginstructions g on a.strBillingInstCode=g.strBillingInstCode  "
			+"left outer join tblbookingtype h on a.strBookingTypeCode=h.strBookingTypeCode "
			+"left outer join tblreceipthd i on a.strReservationNo=i.strRegistrationNo  "
			+"left outer join tblguestmaster j on j.strGuestCode=b.strGuestCode  "
		...
		expectedArrivalListBean.setStrGuestCode(strGuestCode);
		 ...
		 String sqlExpectedArrivalDtl = "select a.strFirstName,a.strMiddleName,a.strLastName,b.strRoomTypeDesc,a.strAddress,a.strArrivalFrom,a.strProceedingTo "
				 					   +"from tblguestmaster a,tblroomtypemaster b,tblreservationhd c,tblroom d,tblreservationdtl e "
                                       +" where  date(c.dteArrivalDate) between '"+fromDate+"' and '"+toDate+"'  and c.strReservationNo='"+strReservationNo+"' and c.strReservationNo=e.strReservationNo and e.strGuestCode=a.strGuestCode and e.strRoomNo=d.strRoomCode and " 
                                       +"d.strRoomTypeCode=b.strRoomTypeCode "; 
