...
public class clsTaxReportController
{
    ...
    public void funGenerateFolioPrintingReport(@RequestParam("fromDate") String fromDate, @RequestParam("toDate") String toDate, HttpServletRequest req, HttpServletResponse resp)
    {
		...
		//get all guests by registration
		    ...
		    sbSql.setLength(0);
		    sbSql.append("select b.strRegistrationNo,c.strGuestPrefix,c.strFirstName,c.strMiddleName,c.strLastName "
		    	+ ",DATE_FORMAT(b.dteArrivalDate,'%d-%m-%Y')as CheckInDate,DATE_FORMAT(b.dteDepartureDate,'%d-%m-%Y') as CheckOutDate"
		    	+ ",a.strRoomNo,e.strRoomTypeDesc "
		    	+ " from tblcheckindtl a "
		    	...
		    	+ " left outer join tblroomtypemaster e on d.strRoomTypeCode=e.strRoomTypeCode "
		    	+ " where date(b.dteArrivalDate) between '" + fromDate + "' and '" + toDate + "' and a.strPayee='Y' ");
			...
			// get tax detail by folio generated
				...
				sbSql.setLength(0);
			    sbSql.append("select a.strRegistrationNo,DATE_FORMAT(b.dteDocDate,'%d-%m-%Y'),c.strDocNo,a.strFolioNo,'NA',d.strIncomeHeadDesc"
					+ ",c.strTaxDesc,c.dblTaxAmt "
					+ " from tblfoliohd a, tblfoliodtl b, tblfoliotaxdtl c, tblincomehead d "
					+ " where a.strFolioNo=b.strFolioNo and  b.strFolioNo=c.strFolioNo and b.strRevenueCode=d.strIncomeHeadCode "
					+ " and b.strDocNo=c.strDocNo and a.strRegistrationNo='" + registrationNo + "' "
					+ " and b.strRevenueType='Income Head'");
			...
			// get tax detail by bill generated				
				sbSql.setLength(0);
			    sbSql.append("select a.strRegistrationNo,DATE_FORMAT(b.dteDocDate,'%d-%m-%Y'),c.strDocNo,a.strFolioNo,a.strBillNo"
			    	+ " ,d.strIncomeHeadDesc,c.strTaxDesc,c.dblTaxAmt "
			    	+ " from tblbillhd a, tblbilldtl b, tblbilltaxdtl c, tblincomehead d "
			    	+ " where a.strBillNo=b.strBillNo and b.strBillNo=c.strBillNo and b.strDocNo=c.strDocNo "
			    	+ " and b.strRevenueCode=d.strIncomeHeadCode and a.strRegistrationNo='" + registrationNo + "' "
			    	+ " and b.strRevenueType='Income Head' ");
			...
			// get tax detail by folio generated for Rooms				
				sbSql.setLength(0);
			    sbSql.append("select a.strRegistrationNo,DATE_FORMAT(b.dteDocDate,'%d-%m-%Y'),c.strDocNo,a.strFolioNo,'NA'"
			    	+ ",b.strPerticulars,c.strTaxDesc,c.dblTaxAmt "
					...
					+ " and a.strRegistrationNo='" + registrationNo + "' and (b.strRevenueType='Room' or b.strRevenueType='ExtraBed') ");
	    ...
	    // income head wise summary
		    ...
		    sbSql.setLength(0);
		    sbSql.append("select b.strRevenueType,sum(c.dblTaxAmt) "
		    	+ " from tblbillhd a, tblbilldtl b, tblbilltaxdtl c "
		    	+ " where a.strBillNo=b.strBillNo and b.strDocNo=c.strDocNo and b.strBillNo=c.strBillNo "
		    ...
		    sbSql.append("select b.strRevenueType,sum(c.dblTaxAmt) "
		    	+ " from tblfoliohd a, tblfoliodtl b, tblfoliotaxdtl c "
		    	+ " where a.strFolioNo=b.strFolioNo and b.strDocNo=c.strDocNo and b.strFolioNo=c.strFolioNo "
		    	+ " and date(b.dteDocDate) between '" + fromDate + "' and '" + toDate + "' "
		    	+ " group by b.strRevenueType ");
		...
		// tax wise summary
			...
			sbSql.setLength(0);
		    sbSql.append("select c.strTaxDesc,sum(c.dblTaxAmt) "
		    	+ " from tblbillhd a left outer join tblbilldtl b on a.strBillNo=b.strBillNo "
		    	+ " left outer join tblbilltaxdtl c on b.strDocNo=c.strDocNo and b.strBillNo=c.strBillNo "
		    ...
		    List listTaxWiseSummaryForFolio = objGlobalFunctionsService.funGetDataList("select c.strTaxDesc,sum(c.dblTaxAmt) "
	            +"from tblfoliohd a left outer join tblfoliodtl b on a.strFolioNo=b.strFolioNo "
	            +"left outer join tblfoliotaxdtl c on b.strDocNo=c.strDocNo and b.strFolioNo=c.strFolioNo "
