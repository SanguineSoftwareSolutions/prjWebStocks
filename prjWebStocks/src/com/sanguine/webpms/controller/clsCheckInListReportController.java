...
public class clsCheckInListReportController {
	...
	public void funGenerateCheckInListReport(@RequestParam("fromDate") String fromDate, @RequestParam("toDate") String toDate,HttpServletRequest req, HttpServletResponse resp)
	{
		try
		{
		    ...
		    String propNameSql="select a.strPropertyName from dbwebmms.tblpropertymaster a "
		    	+ " where a.strPropertyCode='"+propertyCode+"' and a.strClientCode='"+clientCode+"' ";
			...
			// get all parameters
		    String sqlParametersCheckInList = " SELECT ch.strReservationNo, IFNULL(h.strBookingTypeDesc,'NA'), "
		    		+ " DATE_FORMAT(ch.dteDateCreated,'%d-%m-%Y'),IFNULL(c.strCorporateDesc,'NA'), "
		    		+ " IFNULL(k.strBookerName,'NA'), DATE_FORMAT(a.dteCancelDate,'%d-%m-%Y'), "
		    		+ " IFNULL(f.strDescription,'NA'),IFNULL(g.strBillingInstDesc,'NA'), "
		    		+ " CONCAT(j.strFirstName,' ',j.strMiddleName,' ',j.strLastName),j.strGuestCode,ch.strCheckInNo,sum(i.dblReceiptAmt) "
		    		+ " FROM tblcheckinhd ch "
		    		+ " LEFT OUTER JOIN tblreservationhd a ON a.strReservationNo = ch.strReservationNo and a.strClientCode='"+clientCode+"' "
		    		+ " LEFT OUTER JOIN tblreservationdtl b ON a.strReservationNo=b.strReservationNo and b.strClientCode='"+clientCode+"' "
		    		+ " LEFT OUTER JOIN tblcorporatemaster c ON a.strCorporateCode=c.strCorporateCode and c.strClientCode='"+clientCode+"' "
		    		+ " LEFT OUTER JOIN tblbusinesssource f ON a.strBusinessSourceCode=f.strBusinessSourceCode and f.strClientCode='"+clientCode+"' "
		    		+ " LEFT OUTER JOIN tblbillinginstructions g ON a.strBillingInstCode=g.strBillingInstCode and g.strClientCode='"+clientCode+"' "
		    		+ " LEFT OUTER JOIN tblbookingtype h ON a.strBookingTypeCode=h.strBookingTypeCode and h.strClientCode='"+clientCode+"' "
		    		+ " LEFT OUTER JOIN tblreceipthd i ON ch.strCheckInNo=i.strCheckInNo And i.strAgainst='Check-In' and i.strClientCode='"+clientCode+"' "
		    		+ " LEFT OUTER JOIN tblguestmaster j ON j.strGuestCode=b.strGuestCode and j.strClientCode='"+clientCode+"' "
		    		+ " LEFT OUTER JOIN tblbookermaster k ON k.strBookerCode=a.strBookerCode AND k.strClientCode='"+clientCode+"' "
		    		+ " WHERE DATE(a.dteArrivalDate) BETWEEN '"+fromDate+"' and '"+toDate+"' "
		    		+ " AND ch.strClientCode='"+clientCode+"' AND a.strPropertyCode='"+propertyCode+"' "
		    		+ " group by ch.strReservationNo ";
		    ...
		    for (int i = 0; i < listOfCheckIn.size(); i++)
			{
				...
				String sqlCheckInListDtl = " select a.strFirstName,a.strMiddleName,a.strLastName,b.strRoomTypeDesc,a.strAddress,"
			 		+ " a.strArrivalFrom,a.strProceedingTo"
			 		+ " from tblguestmaster a,tblroomtypemaster b,tblcheckinhd c,tblroom d,tblcheckindtl e "
			 		+ " where  date(c.dteArrivalDate) between '"+fromDate+"' and '"+toDate+"' and c.strCheckInNo='"+strCheckInCode+"' "
