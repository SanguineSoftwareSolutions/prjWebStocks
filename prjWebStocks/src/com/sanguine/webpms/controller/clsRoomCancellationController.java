...
// Save or Update RoomCancellation
    ...
    public ModelAndView funAddUpdate(@ModelAttribute("command") @Valid clsRoomCancellationBean objBean, BindingResult result, HttpServletRequest req)
    {
		if (!result.hasErrors())
		{
		    ...
		    objHdModel.setDteCancelDate(objGlobal.funGetCurrentDateTime("yyyy-MM-dd"));
	...
	public @ResponseBody List funLoadHdData(HttpServletRequest request){
		...
		String sql="select a.strReservationNo,ifnull(c.strCorporateCode,'NA'),ifnull(c.strCorporateDesc,'NA' ) "
			+ " ,concat(d.strFirstName,' ',d.strMiddleName,' ',d.strLastName),e.strRoomDesc,b.strGuestCode "
			+ " from tblreservationhd a left outer join tblreservationdtl b on a.strReservationNo=b.strReservationNo "
    ...
    private clsRoomCancellationModel funPrepardBean(clsRoomCancellationBean objBean, HttpServletRequest req, clsReservationHdModel objHdModel)
    {
    	...
    	List listProperty =objGlobalFunctionsService.funGetDataList("select strPropertyCode from dbwebmms.tblpropertymaster where strPropertyName='"+objBean.getStrPropertyCode()+"' ","sql" );
