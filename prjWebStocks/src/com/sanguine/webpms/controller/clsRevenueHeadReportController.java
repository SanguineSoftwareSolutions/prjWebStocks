...
// Open Folio Printing
    ...
    {    	
	    ...
	    // income head types for booked
		    sbSql.setLength(0);
			sbSql.append("SELECT b.strRevenueType, sum(b.dblDebitAmt) "
			...
			// get dtl for this incomeHeadTypeName
				...
				sbSql.append("SELECT c.strTaxDesc,  sum(c.dblTaxAmt) "
					...
					+ " and b.strRevenueType='" + incomeHeadTypeName + "' "
		...
		// get receipts detail
		    ...
		    sbSql.setLength(0);
			sbSql.append("select a.strAgainst,sum(a.dblReceiptAmt) "
				+ " from tblreceipthd a "
				+ " where date(a.dteReceiptDate) between '" + fromDate + "' and '" + toDate + "' "
				+ " group by a.strAgainst ");
		...
		// get collection break ups detail
		    ...
		    List listCollBreakup = objGlobalFunctionsService.funGetDataList("select a.strSettlementDesc,b.dblSettlementAmt "
	            +"from tblsettlementmaster a "
	            +"left outer join tblreceiptdtl b  on a.strSettlementCode=b.strSettlementCode "
	            +"left outer join tblreceipthd c on b.strReceiptNo=c.strReceiptNo "
	            +"where date(c.dteReceiptDate) BETWEEN '" + fromDate + "' and '" + toDate + "' "
