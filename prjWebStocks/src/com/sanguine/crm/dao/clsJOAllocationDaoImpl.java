...
public class clsJOAllocationDaoImpl implements clsJOAllocationDao{
	...
	public  clsJOAllocationHdModel funGetJAHdObject(String strJACode,String clientCode){
		...
		try{
			Query query = sessionFactory.getCurrentSession().createQuery("from clsJOAllocationHdModel a  where a.strJACode = '"+strJACode
	...
	public void funDeleteDtl(String strJAcode, String clientCode) {
		try{
			Query query = sessionFactory.getCurrentSession().createQuery("delete clsJOAllocationDtlModel a  where a.strJACode = '"+strJAcode
					+"' and  a.strClientCode ='"+clientCode+"'");
			query.executeUpdate();
		}catch(Exception e){
			e.printStackTrace();
	...
	public List funGetJAHdData(String strJAcode, String clientCode) {
		...
		try{
			Query query = sessionFactory.getCurrentSession().createQuery("from clsJOAllocationHdModel a ,clsPartyMasterModel b"
						+ " where a.strJACode = '"+strJAcode+"' and  a.strSCCode=b.strPCode "
	...
	public List funGetJADtlData(String strJAcode, String clientCode) {
		List ls= new ArrayList();
		try{
			Query query = sessionFactory.getCurrentSession().createQuery("from clsJOAllocationDtlModel a ,clsJobOrderModel b,clsProductMasterModel c "
						+ " where a.strJACode = '"+strJAcode+"' and a.strJOCode=b.strJOCode "
