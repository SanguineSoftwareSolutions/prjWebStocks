...
public class clsCRMSettlementMasterDaoImpl implements clsCRMSettlementMasterDao{
	...
	@Override
	public clsSettlementMasterModel funGetObject(String strCode,
			String clientCode) {
		List list = new ArrayList();
		try{
			Query query = sessionFactory.getCurrentSession().createQuery("from clsSettlementMasterModel where strSettlementCode = '"
					+strCode+"' and strClientCode ='"+clientCode+"'");
			list = query.list();
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public clsSettlementMasterModel funGeSettlementObject(String strCode,String dteInvDate,String clientCode)
	{
		...
		try{
			Query query = sessionFactory.getCurrentSession().createQuery("from clsSettlementMasterModel where strSettlementCode = '"
					+strCode+"' and  dteInvDate='"+dteInvDate+"' and strClientCode ='"+clientCode+"'");
			list = query.list();
		}catch(Exception e){
			e.printStackTrace();
		}
