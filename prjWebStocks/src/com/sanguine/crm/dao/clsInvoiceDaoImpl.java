...
public class clsInvoiceDaoImpl implements clsInvoiceDao {
	...
	{
		return (clsInvoiceHdModel) sessionFactory.getCurrentSession().get(clsInvoiceHdModel.class,new clsInvoiceHdModel_ID(invCode,clientCode));
	}
	...
	{
		Query query = sessionFactory.getCurrentSession().createSQLQuery("delete from tblinvoicedtl "
	...
	public List <Object> funGetInvoice(String invCode,String clientCode)
	{
		...
		Query query = sessionFactory.getCurrentSession().createQuery(" from  clsInvoiceHdModel a "
				+ "	,clsLocationMasterModel d ,clsPartyMasterModel e "
							+ "	where a.strInvCode=:invCode and "
	...
	public clsInvoiceHdModel  funGetInvoiceDtl(String invCode,String clientCode)
	{
	 ...
	 @Override
		...
		{
			Query query = sessionFactory.getCurrentSession().createSQLQuery("delete from tblinvoicehd "
					+ " where strInvCode='"+invCode+"' and strClientCode='"+clientCode+"' ");
			query.executeUpdate();
		}
		 ...
		 public List funGetHdList(String fromDate,String  toDate,String clientCode)
		 {
			...
			{
				Query query = sessionFactory.getCurrentSession().createQuery("from clsInvoiceHdModel a "
						+ "where a.strInvCode= :saCode and a.strClientCode= :clientCode ");
			...
			{ 
				Query query = sessionFactory.getCurrentSession().createQuery("from clsInvoiceModelDtl a "
						+ "where a.strInvCode = :saCode ");
				query.setParameter("saCode", SACode);
