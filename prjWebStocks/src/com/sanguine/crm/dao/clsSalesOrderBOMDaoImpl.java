...
public class clsSalesOrderBOMDaoImpl implements clsSalesOrderBOMDao{
	...
	public List funGetListOfMainParent(String soCode,String clientCode){
		...
		try{
			Query query = sessionFactory.getCurrentSession().createQuery("from clsSalesOrderBOMModel a,clsProductMasterModel b where "
						+ "a.strSOCode = '"+soCode+"' and b.strProdCode= a.strParentCode and a.strProdCode=b.strProdCode and b.strClientCode ='"+clientCode
						+"' and a.strClientCode ='"+clientCode+"' group by a.strParentCode  ");
			list = query.list();
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public List funGetListOnProdCode(String soCode, String prodCode,String clientCode) {
		...
		try{
			Query query = sessionFactory.getCurrentSession().createQuery("from clsSalesOrderBOMModel a,clsProductMasterModel b where "
						+ "a.strSOCode = '"+soCode+"' and a.strParentCode= '"+prodCode+"' and a.strChildCode=b.strProdCode and b.strClientCode ='"+clientCode
						+"' and a.strClientCode ='"+clientCode+"' ORDER BY a.intindex ASC");
			list = query.list();
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public List funGetListOfSOBom(String soCode,String clientCode)
	{
		...
		try{
			Query query = sessionFactory.getCurrentSession().createQuery("from clsSalesOrderBOMModel a,clsProductMasterModel b where "
						+ "a.strSOCode = '"+soCode+"' and a.strChildCode=b.strProdCode and b.strClientCode ='"+clientCode+"' "
						+ " and a.strClientCode ='"+clientCode+"' and b.strProdType != 'Sub-Contracted'  ");
			list = query.list();
		}catch(Exception e){
			e.printStackTrace();
		}
