...
public class clsBillwiseTaxFlashController {
	...
	private  ModelAndView funExportBillwisetaxDataFlash(@RequestParam(value="fromDate") String fDate,
			@RequestParam(value="toDate") String tDate, @RequestParam(value="strLocCode") String strLocCode,
			@RequestParam(value="strHSN") String strHSN,
			HttpServletRequest req,HttpServletResponse resp)
	{
		...
		/*String sql = " select a.strInvCode,DATE_FORMAT(a.dteInvDate,'%d-%m-%Y'),b.strTaxCode,c.strTaxDesc,b.dblTaxAmt "
		...
		String sql= "  select a.strInvCode,DATE_FORMAT(a.dteInvDate,'%d-%m-%Y'),b.strDocNo,c.strTaxDesc, "
				+ "  b.dblValue,f.strExciseChapter "
				+ " from tblinvoicehd a, tblinvprodtaxdtl b,tbltaxhd c ,tblproductmaster e,tblsubgroupmaster f  "
				+ " where a.strInvCode=b.strInvCode    and c.strTaxCode=b.strDocNo and b.strProdCode = e.strProdCode  "
				+ " and e.strSGCode=f.strSGCode   and a.strLocCode='"+strLocCode+"'  "
				+ "   and  date(a.dteInvDate) BETWEEN  '"+fDate+"' and '"+tDate+"'   "
				+ "  and a.strClientCode='"+clientCode+"' and  b.strClientCode='"+clientCode+"' "
				+ " and c.strClientCode='"+clientCode+"'  "
				+ "  order by a.dteInvDate,a.strInvCode,b.strDocNo  ";
...
//		NavigableSet<String> stTaxDesc = new TreeSet<String>();
		...
		for( String strBill : stBill )
		{
			...
			String sqlBill = " select a.strInvCode,DATE_FORMAT(a.dteInvDate,'%d-%m-%Y') "
					+ " from tblinvoicehd a "
					+ " where "
					+ " a.strLocCode='"+strLocCode+"' "
					+ " and a.strInvCode='"+strBill+"' "
					+ " group by a.strInvCode ";
			...
			for( String strTaxCode : stTaxCode )
			{
					String sqlTaxBill = "  select a.strInvCode,DATE_FORMAT(a.dteInvDate,'%d-%m-%Y'),b.strTaxCode,c.strTaxDesc, "
							+ " b.dblTaxAmt  from tblinvoicehd a, tblinvtaxdtl b,tbltaxhd   c "
							+ " where a.strInvCode=b.strInvCode    "
							+ " and c.strTaxCode=b.strTaxCode and a.strLocCode='"+strLocCode+"' "
							+ " and a.strInvCode='"+strBill+"' and b.strTaxCode='"+strTaxCode+"' "
	...
	private  @ResponseBody List funShowBillwisetaxDataFlash(@RequestParam(value="fromDate") String fDate,
			@RequestParam(value="toDate") String tDate, @RequestParam(value="strLocCode") String strLocCode,
			@RequestParam(value="strHSN") String strHSN,
			HttpServletRequest req,HttpServletResponse resp)
	{
