			...
			public @ResponseBody clsDeliveryChallanBean funAssignFields(@RequestParam("dcCode") String dcCode,HttpServletRequest req)
			{
				...
				for(int i=0;i<objDCDtlModelList.size();i++)
				{
					...
					String sqlHd="select b.dteInvDate,a.dblUnitPrice,b.strInvCode  from tblinvoicedtl a,tblinvoicehd b "
							+" where a.strProdCode='"+dcDtl.getStrProdCode()+"' and a.strClientCode='"+dcDtl.getStrClientCode()+"' and b.strInvCode=a.strInvCode " 
							+" and a.strCustCode='"+objHDModel.getStrCustCode()+"' and  b.dteInvDate=(SELECT MAX(b.dteInvDate) from tblinvoicedtl a,tblinvoicehd b "
							+" where a.strProdCode='"+dcDtl.getStrProdCode()+"' and a.strClientCode='"+dcDtl.getStrClientCode()+"' and b.strInvCode=a.strInvCode  "
							+" and a.strCustCode='"+objHDModel.getStrCustCode()+"')" ;
...
//Convert bean to model function
	private clsDeliveryChallanHdModel funPrepareHdModel(clsDeliveryChallanBean objBean,String userCode,String clientCode,HttpServletRequest req){
		...
		long lastNo=0;
		clsDeliveryChallanHdModel objModel;
		
		
		
		String propCode=req.getSession().getAttribute("propertyCode").toString();
		String startDate=req.getSession().getAttribute("startDate").toString();
		clsDeliveryChallanHdModel DCHDModel;
		objGlobal=new clsGlobalFunctions();
		if(objBean.getStrDCCode().trim().length()==0)
		{
			...
			DCHDModel.setDteCreatedDate(objGlobal.funGetCurrentDateTime("yyyy-MM-dd"));
			DCHDModel.setIntid(lastNo);
		}
		
		else
		{	
			clsDeliveryChallanHdModel objDCModel=objDeliveryChallanHdService.funGetDeliveryChallanHd(objBean.getStrDCCode(),clientCode);
			if(null == objDCModel){
				lastNo=objGlobalFunctionsService.funGetLastNo("tbldeliverychallanhd","DCCode","intId", clientCode);
				String year=objGlobal.funGetSplitedDate(startDate)[2];
				String cd=objGlobal.funGetTransactionCode("DC",propCode,year);			
			...
			DCHDModel.setDteLastModified(objGlobal.funGetCurrentDateTime("yyyy-MM-dd"));
	...
	public ModelAndView funDeliveryChallanListForm(Map<String, Object> model,
			HttpServletRequest request)
    {
		...
		if ("2".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmDeliveryChallanList_1", "command",
					new clsReportBean());
		} else {
			return new ModelAndView("frmDeliveryChallanList", "command",
					new clsReportBean());
		}
    }
	...
	public ModelAndView funDeliveryChallanSlipForm(Map<String, Object> model,
			HttpServletRequest request)
    {
		String urlHits = "1";
		try {
		...
		if ("2".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmDeliveryChallanSlip_1", "command",
					new clsReportBean());
		} else {
			return new ModelAndView("frmDeliveryChallanSlip", "command",
					new clsReportBean());
		}
    }
	...
	public void funCallReportDeliveryChallanReport(String DCCode,String type,HttpServletResponse resp,HttpServletRequest req)
	{
		try {
			...
			String sqlHd="select a.strDCCode,a.dteDCDate,a.strSOCode,b.strPName,b.strSAdd1,b.strSAdd2,b.strSCity,b.strSPin,b.strSState,b.strSCountry , ifnull(c.strCustPONo,''),ifnull(date(c.dteCPODate),'')  " 
					+"	from tbldeliverychallanhd a   "
					+ " left outer join tblpartymaster b on a.strCustCode=b.strPCode "
					+ " left outer join   tblsalesorderhd c on a.strSOCode=c.strSOCode "
					+ " where a.strDCCode='"+DCCode+"' "
	...
	private void funCallDeliveryChallanReport(String SOCode,String type,String fromDate,String toDate,String against,HttpServletResponse resp,HttpServletRequest req )
	{
		...
		try{
			...
			String sqlHd="SELECT  DATE(a.dteDCDate),b.strPName,a.strSAdd1,a.strSAdd2,a.strSCity,a.strSState,a.strSCtry,"
					     +" b.strBAdd1,b.strBAdd2,b.strBCity,b.strBState,b.strBPin,b.strBCountry,b.strPName "
						+"FROM tbldeliverychallanhd a,tblpartymaster b "
						+"WHERE a.strCustCode= '"+SOCode+"' AND a.strCustCode=b.strPCode " 
						+"AND a.strClientCode='"+clientCode+"' AND a.strClientCode=b.strClientCode " 
						+"AND a.dteDCDate BETWEEN '"+fromDate+"' and '"+toDate+"'  ";
			...
			if(against.equalsIgnoreCase("Summary"))
			{
			  sqlDtl="select a.strDCCode,a.dteDCDate,a.strCustCode, b.strPName,a.strPONo,a.strPackNo,a.strAgainst,a.strSOCode "
				    +"from tbldeliverychallanhd a,tblpartymaster b where a.strCustCode='"+SOCode+"' and a.strCustCode=b.strPCode "
					+"and a.strClientCode='"+clientCode+"' and a.strClientCode=b.strClientCode and a.dteDCDate between '"+fromDate+"' and '"+toDate+"' "; 
		  
			}
			else{
				sqlDtl="select a.strDCCode ,b.strProdCode,b.strProdName,a.dblQty,a.strRemarks "
					  +"from tbldeliverychallandtl a,tblproductmaster b,tbldeliverychallanhd c "
					  +"where c.strCustCode='"+SOCode+"' and a.strDCCode=c.strDCCode and a.strClientCode='"+clientCode+"' and "
					  +"a.strProdCode=b.strProdCode and c.dteDCDate between '"+fromDate+"' and '"+toDate+"'  " ;
			}
		  JasperDesign jd=JRXmlLoader.load(reportName);
		  JRDesignQuery subQuery = new JRDesignQuery();
		  subQuery.setText(sqlDtl);
	...
	public void funCallDeliveryChallanRetailReport(HttpServletResponse resp,HttpServletRequest req)
	{
	    ...
	    String sqlQuery = " select c.strProdName,c.strProdNameMarathi,b.dblQty,DATE_FORMAT(a.dteDCDate,'%d-%m-%Y'),d.strLocName,a.strPONo ,e.strPName ,c.strPartNo from tbldeliverychallanhd a,tbldeliverychallandtl b ,tblproductmaster c ,tbllocationmaster d,tblpartymaster e "
	    				 +" where a.strDCCode='"+DCCode+"' and a.strDCCode=b.strDCCode and b.strProdCode=c.strProdCode and a.strLocCode=d.strLocCode and  a.strClientCode='"+clientCode+"' and a.strCustCode=e.strPCode ";
