...
public class clsMISDaoImpl implements clsMISDao {
	...
	public clsMISHdModel funGetObject(String code,String clientCode) {
		...
		/*Query query=sessionFactory.getCurrentSession().createQuery("from clsMISHdModel where strMISCode=:MIScode and strClientCode=:clientCode");
	...
	{
		Query query = sessionFactory.getCurrentSession().createQuery("select a.strMISCode,a.strProdCode,b.strProdName,a.strReqCode,"
				+ " a.dblQty,a.dblUnitPrice,a.dblTotalPrice,b.strIssueUOM,a.strRemarks from clsMISDtlModel a,clsProductMasterModel b "
				+ " where a.strProdCode=b.strProdCode and a.strMISCode = :misCode and a.strClientCode= :clientCode and b.strClientCode= :clientCode");
	...
	public List funMISforMRDetails(String strLocFrom,String strLocTo,String strClientCode) {
		...
		String sqlQuery="select a.strReqCode,DATE_FORMAT(a.dtReqDate,'%m-%d-%Y'),b.strLocName as Locationby,c.strLocName as LocationOn,a.strNarration,a.strAuthorise,ifnull(DATE_FORMAT(dtReqiredDate,'%m-%d-%Y'),'') as dtReqiredDate "
				+ " from tblreqhd a"
				+ " left outer join tbllocationmaster b on  b.strLocCode=a.strLocBy and b.strClientCode='"+strClientCode+"' "
				+ " left outer join tbllocationmaster c on  c.strLocCode=a.strLocOn and c.strClientCode='"+strClientCode+"' "
				+ " Where a.strClientCode='"+strClientCode+"' and a.strReqCode IN"
				+ " (select a.strReqCode  from tblreqdtl a left outer join (select b.strReqCode, b.strProdCode, SUM(dblQty) ReqQty "
				+ " from tblmishd a, tblmisdtl b Where a.strMISCode = b.strMISCode and a.strClientCode='"+strClientCode+"'"
				+ " and b.strClientCode='"+strClientCode+"' group by b.strReqCode, b.strProdCode) b "
				+ " on a.strReqCode  = b.strReqCode and a.strProdCode = b.strProdCode "
				+ " where  a.dblQty > ifnull(b.ReqQty,0)) "
				+ " and a.strLocby='"+strLocTo+"' and a.strLocon='"+strLocFrom+"' and a.strClientCode='"+strClientCode+"' and a.strAuthorise='Yes' and a.strCloseReq='N' order by a.dtReqDate";
	...
	public List<String> funGetProductDtl(String strProdCode, String clientCode) {
		Query query=sessionFactory.getCurrentSession().createQuery("select strProdName,strIssueUOM,strExpDate "
				+ "from clsProductMasterModel where strProdCode=:strProdCode and strClientCode=:clientCode");
