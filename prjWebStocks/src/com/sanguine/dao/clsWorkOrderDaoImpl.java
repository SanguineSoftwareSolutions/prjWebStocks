...
//		return (clsWorkOrderHdModel) sessionFactory.getCurrentSession().get(clsWorkOrderHdModel.class, code);
//	}
	...
	{
		Query query = sessionFactory.getCurrentSession().createQuery("from clsWorkOrderDtlModel a, clsProdProcessModel b, clsProcessMasterModel c "
				      + "where a.strWOCode = :woCode and  a.strProcessCode=b.strProcessCode and b.strProcessCode=c.strProcessCode " 
				      + "and b.strProdProcessCode=:prodCode and a.strClientCode= :clientCode");
...
//			  
//			  clsWorkOrderDtlModel objDtlWorkOrder=(clsWorkOrderDtlModel)listProcessDtl2.get(0);
//			  strProcessCode=objDtlWorkOrder.getStrProcessCode();
			  ...
			  sql="select w.strProcessCode,strProcessName,strStatus,'"+pendingQty+"' as PendingQty"
			  		+ " from tblworkorderdtl w left outer join tblprocessmaster p on w.strProcessCode=p.strProcessCode and p.strClientCode='"+strClientCode+"' "
			  		+ " where w.strWOCode='"+strWOCode+"' and  w.strClientCode='"+strClientCode+"'";
			  listProcessDtl2=sessionFactory.getCurrentSession().createSQLQuery(sql).list();
		  }
		  else
		  {
			  sql="select pp.strprocesscode,p.strProcessName,'Work In Progress','"+qty+"' "
			  		+ "from tblprodprocess pp left outer join tblprocessmaster p on p.strProcessCode=pp.strProcessCode and  p.strClientCode='"+strClientCode+"'"
			  		+ "where strprodcode=(select strProdcode from tblworkorderhd where strwocode='"+strWOCode+"' and strClientCode='"+strClientCode+"') and strClientCode='"+strClientCode+"'";
			  query=sessionFactory.getCurrentSession().createSQLQuery(sql);
			  listProcessDtl2=query.list();
		  }
	...
	public double funQtyPendingForPdn(String strProdCode, String strProcessCode, String strWOCode,String strClientCode)
	{
		...
		String sql="SELECT IFNULL(sum(pd.dblQtyProd-pd.dblQtyRej),0) FROM tblproductiondtl AS pd INNER JOIN tblproductionhd AS ph ON pd.strPDCode = ph.strPDCode and ph.strClientCode='"+strClientCode+"' and ph.strWOCode='"+strWOCode+"'"
				+ " where pd.strProdCode='"+strProdCode+"' and pd.strProcessCode='"+strProcessCode+"' and  pd.strClientCode='"+strClientCode+"'";
				query=sessionFactory.getCurrentSession().createSQLQuery(sql);
				result1=query.list();
				if(result1!=null)
				{
	...
	public String funGetProdProcessStatus(String strProdCode,String strProcessCode, String strWoCode,String strClientCode) {
		...
		String strStatus="";
		sql="SELECT IFNULL(sum(pd.dblQtyProd-pd.dblQtyRej),0) FROM tblproductiondtl AS pd INNER JOIN tblproductionhd AS ph ON pd.strPDCode = ph.strPDCode and ph.strWOCode='"+strWoCode+"' and ph.strClientCode='"+strClientCode+"'  and ph.strAuthorise='Yes' "
				+ " where pd.strProdCode='"+strProdCode+"' and pd.strProcessCode='"+strProcessCode+"'  and pd.strClientCode='"+strClientCode+"'";
	...
	{ 
		int count=0;
		String strStatus="";
		String sql=" select w.strProdCode,p.strProcessCode,w.strWOCode "
				+ " from tblworkorderhd w ,tblworkorderdtl p "
				+ " where w.strWOCode='"+strWOCode+"' and w.strWOCode = p.strWOCode "
				
				+ " and w.strClientCode='"+strClientCode+"' "
				+ " and p.strClientCode='"+strClientCode+"' ";
		List list = sessionFactory.getCurrentSession().createSQLQuery(sql).list();;
        	...
        	sql="SELECT count(distinct '"+Status+"') FROM tblworkorderhd w inner join tblworkorderdtl p ON p.strWOcode = w.strWOcode and p.strClientCode='"+strClientCode+"' "
				+ " where w.strWOCode='"+strWOCode+"' and w.strClientCode='"+strClientCode+"'";
    		...
    		count=Integer.parseInt(Countlist.get(0).toString());
				if (count==1)
				{
					sql="SELECT distinct  '"+Status+"'"
							+ " FROM tblworkorderhd w inner join tblworkorderdtl p ON p.strWOcode = w.strWOcode and p.strClientCode='"+strClientCode+"'"
							+ "	 where w.strWOCode='"+strWOCode +"'  and w.strClientCode='"+strClientCode+"'";
					query = sessionFactory.getCurrentSession().createSQLQuery(sql);
	...
	public List funGenearteWOAgainstOPData(String oPCode,String soCCode ,String strClientCode,String against) {
		...
		if(soCCode.length()>0)
		{
			sql=" select c.strProdCode,c.dblQty from tblproductionorderhd a,tblsalesorderhd b,tblsalesorderdtl c "
					+ " where a.strCode=b.strSOCode and b.strSOCode=c.strSOCode and b.strSOCode='"+soCCode+"' "
					+ " and b.strCloseSO='Y'  and a.strOPCode='"+strClientCode+"' and a.strClientCode='"+strClientCode+"' "
					+ " and a.strClientCode='"+strClientCode+"'  ";		
		}else
		{
			sql="select b.strProdCode,b.dblQty from tblproductionorderhd a,tblproductionorderdtl  b "
					+ " where a.strOPCode=b.strOPCode and a.strOPCode='"+oPCode+"' and a.strClientCode='"+strClientCode+"' "
					+ " and a.strClientCode='"+strClientCode+"' ";
		}
			...
			sql="select c.strProdCode,c.dblQty from tblsalesorderhd b,tblsalesorderdtl c where b.strSOCode=c.strSOCode and b.strSOCode='"+oPCode+"' "
			   + " and b.strCloseSO='N'  and b.strClientCode='"+strClientCode+"' ";
		}
	...
	public List funGetRecipeList(String strParentCode, String strClientCode) {
		String sql="select b.strBOMCode,b.strChildCode,b.dblQty"
				+ " from tblbommasterhd a,tblbommasterdtl b "
				+ " where a.strParentCode='"+strParentCode+"' and a.strBOMCode=b.strBOMCode "
	...
	String sql="select a.strWOCode,a.strProdCode,c.strProdName,c.strReceivedUOM,a.dblQty, "
			+ " a.strStatus,'Production' from tblworkorderhd a, tblworkorderdtl b,tblproductmaster c"
			+ " where a.strWOCode=b.strWOCode and a.strProdCode=c.strProdCode and a.strSOCode='"+oPCode+"' "
