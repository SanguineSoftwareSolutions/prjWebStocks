...
public class clsRequisitionDaoImpl implements clsRequisitionDao
{
	...
	/*	Query query = sessionFactory.getCurrentSession().createQuery("from clsRequisitionHdModel a,"
	...
	@Override
	public List funGetDtlList(String ReqCode,
			String clientCode) {
...
//		String oldQuery=("SELECT r.strProdCode, p.strPartno as strPartno,p.strProdName,"
...
//				+ " inner join tblproductmaster p ON r.strProdCode = p.strProdCode and strNonStockableItem='No' and p.strClientCode='"+clientCode+"' "
...
//				+ " WHERE (r.dblqty-IFNULL(mis.dblQty,0))>0 AND r.strReqCode='"+reqCode+"' and r.strClientCode='"+clientCode+"' group by r.strProdCode ");
		...
		String oldQuery=" SELECT r.strProdCode, p.strPartno as strPartno,p.strProdName, r.dblqty-IFNULL(mis.dblQty, 0) as dblIssueQty , "
				+" r.dblqty as reqQty,p.dblCostRM, r.strRemarks as strRemarks,p.strIssueUOM as IssueUOM "
				+" FROM tblreqdtl r  LEFT OUTER JOIN (select  strProdcode,sum(IFNULL(dblQty, 0)) as dblQty,strReqCode "
					+ "	from  tblmisdtl  " 
					+ "	where strClientCode='"+clientCode+"' and strReqCode='"+reqCode+"'  group by strProdcode) mis on r.strreqcode=mis.strreqcode "
					+ "	and mis.strprodcode=r.strprodcode "
					+ "	inner join tblproductmaster p ON r.strProdCode = p.strProdCode and strNonStockableItem='No' "
					+ "	and p.strClientCode='"+clientCode+"'   "
					+ "	WHERE (r.dblqty-IFNULL(mis.dblQty,0))>0 "
					+ "	AND r.strReqCode='"+ reqCode +"' and r.strClientCode='"+clientCode+"'   ";
	...
	public List funGenerateAutoReq(String strLocCode, String clientCode,String userCode,String strGCode,String strSGCode,String strSuppCode) 
	{
		...
		{
			sqlQuery="select a.strProdCode,a.strProdName,a.strPartNo,a.dblCostRM,a.dblWeight,b.dblReOrderLevel,"
					+ "b.dblReOrderQty,c.dblClosingStk, ifnull(d.pendingQty,0) as OpenReq ,a.strIssueUOM "
					+ "from tblproductmaster a inner join tblreorderlevel b ON a.strProdCode = b.strProdCode and b.strClientCode='"+clientCode+"' "
					...
					+ "left outer join tblmisdtl d ON  a.strReqCode=d.strReqCode and a.strProdCode = d.strProdCode and d.strClientCode='"+clientCode+"' "
					+ "Where b.strLocby ='"+strLocCode+"' and a.strClientCode='"+clientCode+"' Group By a.strProdCode) d ON a.strProdCode = d.strProdCode "
					+ "where  b.strLocationCode='"+strLocCode+"' and c.dblClosingStk <= b.dblReOrderLevel "
		...
		else if (strSuppCode.trim().length()==0 &&  !strGCode.equalsIgnoreCase("ALL") && strSGCode.equalsIgnoreCase("ALL")  && strLocCode.trim().length()>0)
		{
			sqlQuery="select a.strProdCode,a.strProdName,a.strPartNo,a.dblCostRM,a.dblWeight,b.dblReOrderLevel,"
					...
					+ "inner join tblgroupmaster f on e.strGCode=f.strGCode and f.strGCode='"+strGCode+"' and f.strClientCode='"+clientCode+"' "
		...
		else if (strSuppCode.trim().length()==0 && !strGCode.equalsIgnoreCase("ALL") && !strSGCode.equals("ALL") &&  strLocCode.trim().length()>0)
		{
			sqlQuery="select a.strProdCode,a.strProdName,a.strPartNo,a.dblCostRM,a.dblWeight,b.dblReOrderLevel,"
					...
					+ "left outer join tblmisdtl d ON  a.strReqCode=d.strReqCode and a.strProdCode = d.strProdCode and d.strClientCode='"+clientCode+"' "
					+ "Where b.strLocby ='"+strLocCode+"' and a.strClientCode='"+clientCode+"' Group By a.strProdCode) d ON a.strProdCode = d.strProdCode "
					+" inner join tblsubgroupmaster e on a.strSGCode=e.strSGCode and e.strSGCode='"+strSGCode+"'  and e.strClientCode='"+clientCode+"' "
					+ "inner join tblgroupmaster f on e.strGCode=f.strGCode and f.strGCode='"+strGCode+"' and f.strClientCode='"+clientCode+"' "
					+ "where  b.strLocationCode='"+strLocCode+"' and c.dblClosingStk <= b.dblReOrderLevel  and b.dblReOrderLevel > 0  and a.strClientCode='"+clientCode+"' ";	
	...
	@Override
	public void funDeleteProductStandard(String clientCode)
	{
		try {
			sessionFactory.getCurrentSession().createSQLQuery("Delete from tblproductstandard where strClientCode='"+clientCode+"' ").executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	@Override
	public List<clsProductStandardModel> funGetProductStandartList(String clientCode) {
