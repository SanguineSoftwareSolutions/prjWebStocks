...
public class clsRateContractDaoImpl implements clsRateContractDao
{
	...
	{
		Query query = sessionFactory.getCurrentSession().createQuery("from clsRateContractHdModel a,clsSupplierMasterModel b "
				+ "where a.strRateContNo= :rateContNo and a.strSuppCode=b.strPCode and a.strClientCode= :clientCode");
		query.setParameter("rateContNo", rateContractNo);
		query.setParameter("clientCode", clientCode);
		return query.list();
		//return (clsRateContractHdModel)sessionFactory.getCurrentSession().get(clsRateContractHdModel.class,rateContractNo);
	}
