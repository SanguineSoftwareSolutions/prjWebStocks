...
public class clsProductionDaoImpl implements clsProductionDao {
	...
	public List funGetDtlList(String PDCode,	String clientCode) {
		//select pd.strProdCode,p.strProdName,pd.strProcessCode,pd.dblQtyProd,pd.dblQtyRej,pd.dblWeight,pd.dblPrice,pd.dblActTime 
//		Query query = sessionFactory.getCurrentSession().createQuery("from clsProductionDtlModel where a.strSACode = :saCode and a.strProdCode=b.strProdCode and a.strClientCode= :clientCode");
...
//		Query query = sessionFactory.getCurrentSession().createQuery("from clsWorkOrderHdModel wo,clsWorkOrderDtlModel woDtl ,clsProcessMasterModel Pr"
...
//				+ " and wo.strWOCode=:WoCode and wo.strClientCode=:clientCode");
		List list=null;
		String sql=" select wo.strProdCode,p.strProdName,"
				+ " woDtl.strProcessCode,pr.strProcessName,p.dblWeight,wo.dblQty,p.strUOM,p.dblCostRM "
				+ " from tblworkorderhd wo,tblworkorderdtl woDtl,tblprocessmaster pr, tblproductmaster p "
				+ " where wo.strWOCode ='"+workOrderCode+"' and wo.strWOCode=woDtl.strWOCode "
				+ " and  wo.strProdCode = p.strProdCode  and woDtl.strProcessCode=pr.strProcessCode "
				+ " and wo.strClientCode='"+clientCode+"'";
	...
	@Override
	public int funUpdateWorkOrderStatus(String strWOCode, String strStatus,
			String strUser,String dteModifieddate,String strClientCode) {
	...
	public Double funGetPdProdQty(String workOrderCode,  String ProdCode,String clientCode) {
		String sql="select ifnull(sum(a.dblQtyProd),0) from tblproductiondtl a, tblproductionhd b "
				+ " where a.strPDCode=b.strPDCode and b.strWOCode='"+workOrderCode+"'  and a.strProdCode='"+ProdCode+"' and a.strClientCode='"+clientCode+"' and b.strClientCode='"+clientCode+"' ";
	...
	public List funGetWorkOrdersComplete(String[] woCodes,String clientCode)
	{
		...
		List list=null;
		String sql=" select a.strPDCode from tblproductionhd a "
				+ " where "+codes+" "
