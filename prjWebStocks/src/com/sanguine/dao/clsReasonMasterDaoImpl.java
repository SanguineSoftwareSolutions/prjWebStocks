...
public class clsReasonMasterDaoImpl implements clsReasonMasterDao {
	...
	public Map<String, String> funGetResonList(String clientCode) {
		...
		String hql="from clsReasonMaster where  strClientCode=:clientCode ORDER BY strReasonName";
		Query query=sessionFactory.getCurrentSession().createQuery(hql);
		query.setParameter("clientCode", clientCode);
		List<clsReasonMaster> mapLocation= query.list();
		for(clsReasonMaster reason: mapLocation)
		{
			map.put(reason.getStrReasonCode(), reason.getStrReasonName());
		}
		return map;
	}
