...
public class clsLocationMasterDaoImpl implements clsLocationMasterDao
{
	...
	{ 
		String hql="select b.strLocCode,b.strLocName,a.dblReOrderLevel,a.dblReOrderQty from tbllocationmaster b"
				+ " left outer join tblreorderlevel a on b.strLocCode=a.strLocationCode and b.strClientCode=:clientCode"
				+ " where a.strProdCode=:prodCode and a.strClientCode=:clientCode union all "
				+ " select strLocCode,strLocName,0.00,0.00  from tbllocationmaster "
		...
		//String oldQuery="from clsLocationMasterModel where strLocCode not in(select strLocationCode from clsProductReOrderLevelModel where strProdCode=:prodCode) and strClientCode=:clientCode ";
