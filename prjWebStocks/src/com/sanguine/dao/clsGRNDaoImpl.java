...
public class clsGRNDaoImpl implements clsGRNDao
{
	...
	{
		String hql="UPDATE clsProdSuppMasterModel set dblLastCost = :price, dblMaxQty = :maxQty"
					+"WHERE strSuppCode = :suppCode and strProdCode = :prodCode and strClientCode =: clientCode";
	...
	{
		Query query = sessionFactory.getCurrentSession().createQuery("from clsGRNHdModel a,clsSupplierMasterModel b,clsLocationMasterModel c "
				+ "where a.strGRNCode= :grnCode and a.strSuppCode=b.strPCode and a.strLocCode=c.strLocCode "
	...
	{
		Query query = sessionFactory.getCurrentSession().createQuery("from clsGRNDtlModel a,clsProductMasterModel b "
				+ " where a.strGRNCode = :grnCode and a.strProdCode=b.strProdCode and a.strClientCode= :clientCode"
	...
	{
		/*String strPOCodes="";
		String tempPoCode[]=code.split(",");
		for(int i=0;i<tempPoCode.length;i++)
		{
			if(strPOCodes.length()>0)
			{
				strPOCodes =strPOCodes+" or a.strPOCode='"+ tempPoCode[i]+"' ";
			}
			else
			{
		...
		/*String sql="select c.strProdCode,a.strPOCode,a.dblOrdQty, ifnull(b.POQty,0), a.dblOrdQty - ifnull(b.POQty,0) as BalanceQty,a.dblPrice"
				...
				+ "where a.dblOrdQty > ifnull(b.POQty,0) and a.strPOCode='"+code+"' and a.strClientCode='"+clientCode+"'";
		*/
		String sql="";
		StringBuffer sbsql = new StringBuffer();
		/*sql="select c.strProdCode,a.strPOCode,a.dblOrdQty, ifnull(b.POQty,0), a.dblOrdQty - ifnull(b.POQty,0) as BalanceQty,a.dblPrice , "
				...
				+ " left outer join tblmrpidtl d on d.strPICode=a.strPICode and d.strClientCode='"+clientCode+"'"
				+ " left outer join tblreqhd e on e.strReqCode=d.strReqCode and e.strClientCode='"+clientCode+"'"
				+ " left outer join tbllocationmaster f on f.strLocCode=e.strLocBy and f.strClientCode='"+clientCode+"' "
				+ " where a.dblOrdQty > ifnull(b.POQty,0) and a.strPOCode='"+code+"' and a.strClientCode='"+clientCode+"'";*/
		
		sbsql.append("select c.strProdCode,a.strPOCode,a.dblOrdQty, ifnull(b.POQty,0), a.dblOrdQty - ifnull(b.POQty,0) ,a.dblPrice , "
				+ " a.dblWeight,c.strUOM,c.strProdName,if(LENGTH(c.strExpDate)=0,'N',c.strExpDate),a.dblDiscount,ifnull(f.strLocCode,''), "
				+ " ifnull(f.strLocName,'') ,ifnull(c.strNonStockableItem,'') ,ifnull(e.strReqCode,'') "
				+ " from tblpurchaseorderdtl a left outer join (SELECT b.strCode AS POCode, b.strProdCode, SUM(b.dblQty) AS POQty "
				+ " FROM tblgrnhd a INNER JOIN tblgrndtl b ON a.strGRNCode = b.strGRNCode  and b.strClientCode='"+clientCode+"'"
				+ " WHERE (a.strAgainst = 'Purchase Order')  and a.strClientCode='"+clientCode+"'"
				+ " GROUP BY POCode, b.strProdCode) b on a.strPOCode = b.POCode and a.strProdCode = b.strProdCode "
	...
	{
		Query query = sessionFactory.getCurrentSession().createQuery("from clsTaxHdModel where strTaxType='Fixed Amount' "
	...
	public List funLoadPOforGRN(String strSuppCode,String strPropCode,String strClientCode) {
		/*String sql="select strPOCode,date(dtPODate),strCode from tblPurchaseOrderHd  "
				+ " where strSuppCode='"+strSuppCode+"' and strclosePO='No' ";*/
		String sql="select strPOCode,Date(dtPODate),strAgainst from tblpurchaseorderhd Where strPOCode IN "
				+"(select distinct a.strPOCode from tblpurchaseorderdtl a "
				+ "left outer join (SELECT b.strCode AS POCode, b.strProdCode, SUM(b.dblQty) AS POQty "
				+ "FROM tblgrnhd a INNER JOIN tblgrndtl b ON a.strGRNCode = b.strGRNCode and b.strClientCode='"+strClientCode+"' "
				+ "WHERE (a.strAgainst = 'Purchase Order') and a.strClientCode='"+strClientCode+"' GROUP BY b.strCode, b.strProdCode) b "
				+ "on a.strPOCode = b.POCode and a.strProdCode = b.strProdCode and a.strClientCode='"+strClientCode+"'"
				+ "where  a.dblOrdQty > ifnull(b.POQty,0)) "
				+ "and strClosePO != 'Yes' and strAuthorise='Yes' and strSuppCode='"+strSuppCode+"' and strClientCode='"+strClientCode+"' and strPropCode='"+strPropCode+"'";		
	...
	public List funGetNonStkData(String strPOCode, String strGrnCode,String strClientCode) 
	{
		...
		String sql="select IfNULL(b.strReqCode,'') from tblpurchaseorderdtl a "
				+ " inner join tblmrpidtl b on a.strPIcode =b.strPIcode"
				+ " where a.strPOCode='"+strPOCode+"' group by b.strReqCode";
		Query query = sessionFactory.getCurrentSession().createSQLQuery(sql);		
		...
		String sql1=" SELECT reqhd.strReqCode,DATE_FORMAT(reqhd.dtReqDate,'%m-%d-%Y') as reqDate,reqdtl.strProdCode,p.strProdName,"
				+ " l .strLocName as strLocByName,reqhd.strLocby ,lo.strLocName as strLocToName,reqhd.strLocon,reqdtl.dblQty,reqdtl.strRemarks,"
				+ " p.dblCostRM ,p.dblCostRM*reqdtl.dblQty as price,reqhd.dblSubTotal  ,grnDtl.GRNQty "
				+ " FROM tblreqhd reqhd inner join tblreqdtl reqdtl on reqhd.strReqCode =reqdtl.strReqCode and reqdtl.strClientCode='"+strClientCode+"' "
				+ " INNER JOIN tblproductmaster p ON reqdtl.strProdCode = p.strProdCode and p.strClientCode='"+strClientCode+"' " 
				+ " inner join tbllocationmaster l on l.strLocCode=reqhd.strLocby and l.strClientCode='"+strClientCode+"' "
				+ " inner join tbllocationmaster lo on lo.strLocCode=reqhd.strLocon  and lo.strClientCode='"+strClientCode+"'  "
				+ " left outer Join  (SELECT strProdCode, sum(dblQty-dblRejected) as GRNQty "
				+ " from tblgrndtl where strGRNCode='"+strGrnCode+"'  and strClientCode='"+strClientCode+"' group by strProdCode) grnDtl on grnDtl.strProdCode=reqdtl.strProdCode "
				+ " where reqhd.strReqCode='"+strReqCode+"' and p.strNonStockableItem= 'Yes' and reqhd.strClientCode='"+strClientCode+"' order by p.strProdCode ";
