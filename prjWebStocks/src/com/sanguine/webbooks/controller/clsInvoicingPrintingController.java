...
//		String showBOM = objBean.getStrShowBOM();
	...
	private void funCallReportParticulorInvoicingPrintingReport(String strFromCatCode,String strToCatCode,String innFromDate,String innToDate,String innvFromNo,String innvToNo,String fromAmt,String toAmt,HttpServletResponse resp,HttpServletRequest req)
	{
		try {
			...
			String sqlHdFTData="select a.strHdLine1,a.strHdLine2,a.strHdLine3,a.strFtLine1,a.strFtLine2,a.strFtLine3  "
					+" from tblinvprocesshd a " 
					+" where a.strClientCode='"+clientCode+"' "
					+" and date(a.dteInvDate) between '"+innFromDate+"' and '"+innToDate+"'  ";
			...
			String sqlHd="select a.strDebtorName as Name, a.strAddress1 as add1 ,a.strAddress2 as add2 , a.strAddress3 as add3 ,"
					+" a.strDebtorCode as MemberShipNo ,a.strMonth as Monthly,a.strInvNo as BillNo ,"
					+" a.dteInvDate as BDate ,a.dblOpenBal as DrBal,a.dblTotalDr as DrPeriod,"
					+" a.dblTotalCr as CrPeriod,dblClgBal as closeDRBal,dblOpenBalCr as CRBal "
					+" from tblInvDebtorHd a "
					+" where a.strCategoryCode='"+strFromCatCode+"' "
					+" and a.strInvno ='"+innvFromNo+"'  "
					+" and a.dteInvDate between '"+innFromDate+"' and '"+innToDate+"' "
					+" and a.dblClgBal between '"+fromAmt+"' and '"+toAmt+"'  "
...
//						+" and a.strClientCode='"+clientCode+"' ";
			...
			String sqlDtlCr="select Date(a.dtechqBillDate) as BillDate,a.strChqBillNo as BillNo,a.strParticulars as Particulars, a.dblAmt as Amt "
		    				+" from tblInvDebtorDtl a , tblinvdebtorhd b "
		    				+ "where a.strCrDr='Cr' "
		    				+" and a.strClientCode='"+clientCode+"' and a.strInvno=b.strInvno and b.strInvno ='"+innvFromNo+"' "
            ...
            String sqlDtlDr="select Date(a.dtechqBillDate) as BillDate,a.strChqBillNo as BillNo,a.strParticulars as Particulars, a.dblAmt as Amt "
            				+" from tblInvDebtorDtl a , tblinvdebtorhd b "
            				+ "where a.strCrDr='Dr' "
            				+" and a.strClientCode='"+clientCode+"' and a.strInvno=b.strInvno and b.strInvno ='"+innvFromNo+"' "
            				+" order by a.dtechqBillDate "; 
            ...
            String sqlSummaryData ="select a.strParticulars as Particulars, sum(a.dblAmt) as Amt  "
            				+" from tblInvDebtorDtl a  "
            				+" where a.strInvNo='"+innvFromNo+"' and a.strCrDr='Dr'  "
            				+" group by a.strParticulars order by a.dtechqBillDate ";
...
//         
	...
	private void funCallReportInvoicingPrintingList(String strFromCatCode,String strToCatCode,String innFromDate,String innToDate,String innvFromNo,String innvToNo,String fromAmt,String toAmt,HttpServletResponse resp,HttpServletRequest req)
	{
		try {
			...
			String sqlHdData="select a.strHdLine1,a.strHdLine2,a.strHdLine3,a.strFtLine1,a.strFtLine2,a.strFtLine3  "
					+" from tblinvprocesshd a " 
					+" where a.strClientCode='"+clientCode+"' "
					+" and date(a.dteInvDate) between '"+innFromDate+"' and '"+innToDate+"'  ";
			...
			String sql="select b.strCrDr,a.strDebtorName, a.strAddress1 ,a.strAddress2  , a.strAddress3  , a.strDebtorCode  "
						...
						+" where a.strCategoryCode between '"+strFromCatCode+"' and '"+strToCatCode+"' "
						+" and a.strInvno=b.strInvno  " 
						+" and a.strInvno between '"+innvFromNo+"' and '"+innvToNo+"' " 
						+" and a.dteInvDate  " 
						+" between '"+innFromDate+"' and '"+innToDate+"' "
						+" and a.dblClgBal  " 
