...
public class clsWebBooksAccountMasterDaoImpl implements clsWebBooksAccountMasterDao{
	...
	public List funGetWebBooksAccountMaster(String accountCode,String clientCode)
	{
		...
		try
		{
			Query query = webBooksSessionFactory.getCurrentSession().createQuery("from clsWebBooksAccountMasterModel a,clsACGroupMasterModel b "
							+"where a.strGroupCode=b.strGroupCode  and a.strClientCode=b.strClientCode and a.strClientCode=:clientCode "
							+"and a.strAccountCode=:accountCode ");
	...
	{	
		Query sqlNextAccountNo=webBooksSessionFactory.getCurrentSession().createSQLQuery("select ifnull(max(strAccountCode),CONCAT('"+groupCode+"','-01-','00')) "
				+ "from  tblacmaster "
				+ "where strGroupCode='"+groupCode+"' and strClientCode='"+clientCode+"' ");
