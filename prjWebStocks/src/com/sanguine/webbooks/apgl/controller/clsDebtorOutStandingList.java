...
public class clsDebtorOutStandingList {
	...
	public ModelAndView funOpenForm(Map<String,Object> model,HttpServletRequest request)
	{
		...
		if ("2".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmDebtorOutStandingList_1",
					"command", new clsCreditorOutStandingReportBean());
		} else if ("1".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmDebtorOutStandingList", "command",
			...
			try {
				...
				if (jprintlist.size() > 0) {
					...
					exporter.setParameter(JRPdfExporterParameter.IGNORE_PAGE_MARGINS,Boolean.TRUE);
					 ...
					 "inline;filename=rptDebtorOutStandingList"+dteFromDate+"_To_"+dteToDate+"_"+userCode+".pdf");
					...
					exporter.setParameter(JRXlsExporterParameter.IGNORE_PAGE_MARGINS,Boolean.TRUE);
					 resp.setHeader("Content-Disposition",
					 "inline;filename=rptDebtorOutStandingList"+dteFromDate+"_To_"+dteToDate+"_"+userCode+".xls");
					exporter.exportReport();
					servletOutputStream.flush();
					servletOutputStream.close();
					 }
