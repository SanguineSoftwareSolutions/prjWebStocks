...
//Open WebClubCommitteeMaster
	...
	public ModelAndView funOpenForm(Map<String, Object> model,HttpServletRequest request){
		...
		{
			return new ModelAndView("frmWebClubCommitteeMaster_1", "command",
					new clsWebClubCommitteeMasterBean());
		}
		else if ("1".equalsIgnoreCase(urlHits))
		{
			return new ModelAndView("frmWebClubCommitteeMaster", "command",
					new clsWebClubCommitteeMasterBean());
		}
		else
		{
...
//Save or Update WebClubCommitteeMaster
	...
	public ModelAndView funAddUpdate(@ModelAttribute("command") @Valid clsWebClubCommitteeMasterBean objBean ,BindingResult result,HttpServletRequest req){
		if(!result.hasErrors()){
			...
			//clsWebClubCommitteeMasterDtl objModelDtl =funPrepareDtlModel(objBean,userCode,objModel.getStrCommitteeCode(),clientCode,req);
