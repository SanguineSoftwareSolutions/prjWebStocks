...
public class clsExciseBillGenerateController{
	...
	public ModelAndView funOpenForm(Map<String, Object> model, HttpServletRequest req){
  	 ...
  	 String Licence_sql="select a.strLicenceNo,CONCAT(a.strAddress1,',',a.strAddress2,"
					+ "',',a.strAddress3,',',b.strCityName,CONCAT('-',a.strPINCode)) as address,a.strLicenceCode "
					+ "from tbllicencemaster a ,tblcitymaster b "
					+ "where a.strCity=b.strCityCode and a.strClientCode='"+clientCode+"' and a.strPropertyCode='"+propoertyCode+"' ";
		...
		if (licenceMaster.size() > 0) {
			...
			req.getSession().setAttribute("successMessage",
					"Please Add Licence To This Client");
		}
	...
	public @ResponseBody List funExciseAbstractReportColumns(HttpServletRequest req){
		...
		String saleData_sql="select a.intId,a.dteBillDate,a.strUserCreated,a.dteDateCreated,a.strLicenceCode,a.strSourceEntry "
				+ " from tblmanualsaleshd a where date(a.dteBillDate) BETWEEN '"+fromDate+"' and '"+toDate+"' and a.strLicenceCode='"+licenceCode+"' "
...
//				String toDate=toDate1.split("-")[2]+"-"+toDate1.split("-")[1]+"-"+toDate1.split("-")[0];
									...
									List objDtlList = objBillGenService.funGetSalesDtlList(objSalesHdModel.getIntId(), clientCode);
										...
										for(int i=0;i<objDtlList.size();i++){
												...
												try
												{
													...
													String sqlRecipe="select a.strParentCode,b.strBrandCode,b.dblQty,c.intPegSize,d.intQty  from tblexciserecipermasterhd a, "
															+" tblexciserecipermasterdtl b,tblbrandmaster c,tblsizemaster d  "
															+" where a.strRecipeCode=b.strRecipeCode and a.strParentCode=c.strBrandCode " 
															+" and c.strSizeCode=d.strSizeCode and a.strParentCode='"+objSalesDtl.getStrBrandCode()+"' and b.strClientCode='"+clientCode+"'" 
															+" and a.strClientCode='"+clientCode+"'";
													...
													if(listRecipeData.size()>0)
													{
														for(int cnt=0;cnt<listRecipeData.size();cnt++)
														{
															...
															dtlBillGen = funGenerateBill(strBrandCode, "0", 
																	"", strtotalInML, 
																	objSalesHdModel.getDteBillDate(), objSalesHdModel.getIntId().toString(),
																	licenceCode,objSalesHdModel.getStrSourceEntry(), req
																);
													...
													dtlBillGen = funGenerateBill(objSalesDtl.getStrBrandCode(), objSalesDtl.getIntBtl().toString(), 
																		objSalesDtl.getIntPeg().toString(), objSalesDtl.getIntML().toString(), 
																		objSalesHdModel.getDteBillDate(), objSalesHdModel.getIntId().toString(),
																		licenceCode,objSalesHdModel.getStrSourceEntry(), req
																	);
													}
	...
	public boolean funGenerateBill(String strBrandCode, String strBtls, String strPeg, String strML,
			String dteBillDate, String strSaleCode, String strLicenceCode,
			String strSourceEntry, HttpServletRequest req)
	{
		String userCode = req.getSession().getAttribute("usercode").toString();
		String clientCode = req.getSession().getAttribute("clientCode").toString();
...
//		if(objSalesDtl.getStrBillGenFlag().equalsIgnoreCase("N")){
		...
		if(!strBrandCode.isEmpty()){
			...
			String sql_BrandList="select a.strBrandCode,a.strSizeCode,a.strShortName,b.intQty,a.intPegSize,  "
					...
					+ " ON d.strBrandCode = a.strBrandCode  AND d.strClientCode='"+clientCode+"',"
					+ " tblsizemaster b,tblsubcategorymaster c "
					+ " where a.strBrandCode='"+strBrandCode+"' "
				...
				String sql_OpData="select c.intOpBtls,c.intOpPeg,c.intOpML from tblbrandmaster a,tblopeningstock c "
									+ " where a.strBrandCode='"+strBrandCode+"' and a.strBrandCode=c.strBrandCode  "
									+ " and  a.strClientCode='"+tempBrandClientCode+"' and  c.strClientCode='"+clientCode+"' and c.strLicenceCode='"+strLicenceCode+"' ";
...
//				System.out.println(brandData[7]);
    			...
    			if(availableStk>=SaleQty){
    				...
    				for (int i = 0; i < cnt; i++) {
    					...
    					result = funAddSaleDataBulkly(saleDevidedList, strBrandCode, 
						    							pegSize, rate, dteBillDate, 
						    							strLicenceCode,  strSaleCode, 
						    							strSourceEntry, clientCode, userCode
		    										);
	...
	@SuppressWarnings("rawtypes")
	private boolean funAddSaleDataBulkly(ArrayList<Integer> saleDevidedList, String strBrandCode, 
											Integer pegSize, Double rate, 
											String dteBillDate, String strLicenceCode, 
											String strSaleCode,  String strSourceEntry,  
...
//			String oneDayPassNoString = " SELECT MAX(CAST(a.strPermitCode AS SIGNED INTEGER)) AS maxNumber "
...
//			}
			...
			String oneDayPassNoString = " SELECT a.strPermitCode AS maxNumber "
					+ " from tblexcisesaledata a  where date(a.dteBillDate) ='"+oneDayPassModel.getDteDate()+"' "
...
//						    String strBillNo=objGlobalFunctions.funGenerateDocumentCode("frmExciseBillGenerate", dteBillDate, req);
							...
							if(oneToMany >=3){
//								String oneToManypermitCode ="select a.intBillNo,a.strPermitCode from tblexcisesaledata a "
...
//										objHdModel.setIntBillNo(Long.parseLong(strPermitArr[0].toString()));
	...
	public void funAuditHdBill(clsExciseManualSaleHdModel objHdModel ,String userCode,String clientCode){
		...
		String countSql="select count(*) from dbwebmms.tblaudithd "
				           + "where left(strTransCode,"+cnt+")='"+objHdModel.getIntId()+"' and strClientCode='"+clientCode+"' ";
		...
		if(cnt==0){
		  ...
		  int cnt=count.length(); 
		  String countSql="select count(*) from dbwebmms.tblaudithd "
				           + "where left(strTransCode,"+cnt+")='"+objDtlModel.getStrSalesCode()+"' and strClientCode='"+objDtlModel.getStrClientCode()+"'";
