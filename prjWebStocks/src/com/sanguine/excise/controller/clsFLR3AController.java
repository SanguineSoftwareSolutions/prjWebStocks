 ...
 */
	...
	public void funSaveData(@ModelAttribute("command") @Valid clsFLR3ABean objBean,BindingResult result, 
	...
	public @ResponseBody HashMap funExciseReportColumn(HttpServletRequest req){
		...
		if(!licenceCode.equalsIgnoreCase("All")){
		 Licence_sql="select a.strLicenceNo,CONCAT(a.strAddress1,',',a.strAddress2,"
				+ "',',a.strAddress3,',',b.strCityName,CONCAT('-',a.strPINCode)) as address,a.strLicenceCode "
				+ "from tbllicencemaster a ,tblcitymaster b "
				+ "where a.strCity=b.strCityCode and a.strClientCode='"+clientCode+"' and a.strLicenceCode='"+licenceCode+"' and a.strPropertyCode='"+propertyCode+"'";
		}else{
			Licence_sql="select a.strLicenceNo,CONCAT(a.strAddress1,',',a.strAddress2,"
					+ "',',a.strAddress3,',',b.strCityName,CONCAT('-',a.strPINCode)) as address,a.strLicenceCode "
					+ "from tbllicencemaster a ,tblcitymaster b "
...
//		String sql_SizeList="select strSizeCode,intQty from tblsizemaster where "
...
//		String sql_SizeList="select DISTINCT a.strSizeCode,a.strSizeName,a.intQty,c.strIsDecimal from tblsizemaster a,tblbrandmaster b,tblsubcategorymaster c  "
...
//				+ tempSizeClientCode+"' and b.strClientCode='"+tempBrandClientCode+"' ORDER BY intQty DESC";
		...
		String sql_SizeList="select DISTINCT a.strSizeCode,a.strSizeName,a.intQty,c.strIsDecimal from tblsizemaster a,tblbrandmaster b,tblsubcategorymaster c  "
				...
				+ " p.strBrandCode=m.strBrandCode AND m.strSubCategoryCode='"+subCatCode+"' "
				+ " UNION ALL  "
				+ " (SELECT q.strBrandCode FROM tblopeningstock q,tblbrandmaster n WHERE q.strClientCode='"+clientCode+"' AND "
				+ " q.strBrandCode=n.strBrandCode AND n.strSubCategoryCode='"+subCatCode+"')) AS strBrandCode) AND"
				+ " a.strSizeCode=b.strSizeCode AND b.strSubCategoryCode='"+subCatCode+"' AND b.strSubCategoryCode=c.strSubCategoryCode AND "
	...
	public LinkedList<LinkedList<String>> funExciseStkManupulation(String subCatCode,String fromDate1,String toDate1,HttpServletRequest req){
		...
		try{
			...
			if(isBrandGlobal.equalsIgnoreCase("All")){
			 ...
			 String sql_BrandList="select a.strBrandCode,a.strSizeCode,a.strShortName,b.intQty,a.intPegSize,a.strBrandName "
						+ " from tblbrandmaster a, tblsizemaster b  "
						+ " where a.strBrandCode IN "
						+ " (SELECT DISTINCT strBrandCode FROM "
						+ " (SELECT p.strBrandCode FROM tbltpdtl p,tblbrandmaster m WHERE p.strClientCode='"+clientCode+"' AND"
						+ " p.strBrandCode=m.strBrandCode AND m.strSubCategoryCode='"+subCatCode+"' "
						+ " UNION ALL  "
						+ " (SELECT q.strBrandCode FROM tblopeningstock q,tblbrandmaster n WHERE q.strClientCode='"+clientCode+"' AND "
						+ " q.strBrandCode=n.strBrandCode AND n.strSubCategoryCode='"+subCatCode+"')) AS strBrandCode) AND "
						+ " a.strSizeCode=b.strSizeCode AND a.strSubCategoryCode='"+subCatCode+"' "
...
//						System.out.println(objBrandData[5]);
						...
						if(!licenceCode.equalsIgnoreCase("All")){
							 sql_OpData="select c.intOpBtls,c.intOpPeg,c.intOpML from tblbrandmaster a,tblopeningstock c "
										...
										+ " and  a.strClientCode='"+tempBrandClientCode+"' and  c.strClientCode='"+clientCode+"' and c.strLicenceCode='"+licenceCode+"' ";	
						}else{
							sql_OpData="select ifnull(sum(c.intOpBtls),'0'), ifnull(sum(c.intOpPeg),'0'), ifnull(sum(c.intOpML),'0') from tblbrandmaster a,tblopeningstock c "
									+ " where a.strBrandCode='"+ls.get(0)+"' and a.strBrandCode=c.strBrandCode  "
...
//		
	...
	public LinkedList<String> funStockList(List objBrandData,HttpServletRequest req,String fromDate,String toDate){
		...
		//check for Licence code is not set in session show data only client wise
		try{
		...
		// Check For is Brand Has Recipe Here Only those brand allow which has no recipe 
		
		String sql_RecipeList="select distinct strParentCode FROM tblexciserecipermasterhd a "
				+ "where a.strParentCode='"+objBrandData.get(0).toString()+"' and a.strClientCode='"+clientCode+"' ";
	...
	public LinkedHashMap<String,String> funTpStock(String brandCode,HttpServletRequest req,String fromDate,String toDate){
		...
		if((!licenceCode.equalsIgnoreCase("All"))&&(!licenceCode.equalsIgnoreCase(""))){
			sql_TpQty="select a.strTPNo,b.intBottals,a.strTPCode from tbltphd a, tbltpdtl b where b.strTPCode=a.strTPCode and "
	...
	public Long funSalesStk(String brandCode,Integer brandSize,HttpServletRequest req,String fromDate,String toDate,Integer brandPegSize){
			...
			for(int i=0;i<RecipeList.size();i++){
				...
				if(!licenceCode.equalsIgnoreCase("All"))
				{
					sql_SalesQty="select ifnull(sum(a.intTotalPeg),'0') as Qty from tblexcisesaledata a where "
							...
							+ "a.strItemCode='"+recipeObj[0].toString()+"' and a.strClientCode='"+clientCode+"' and a.strLicenceCode='"+licenceCode+"' "; 
					
					
				}else{
					sql_SalesQty="select ifnull(sum(a.intTotalPeg),'0') as Qty from tblexcisesaledata a where "
							+ "date(a.dteBillDate) between '"+fromDate+"' and '"+toDate+"' and "
							+ "a.strItemCode='"+recipeObj[0].toString()+"' and a.strClientCode='"+clientCode+"'  ";
		...
		if(!licenceCode.equalsIgnoreCase("All"))
		{
			sql_SalesQty="select ifnull(sum(a.intQty),'0') as Qty from tblexcisesaledata a where "
					+ "date(a.dteBillDate) between '"+fromDate+"' and '"+toDate+"' and "
