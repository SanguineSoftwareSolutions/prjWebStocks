...
public class clsExcisePurchaseReportController {
	...
	//Open PurchaseReport form
		...
		private ModelAndView funPurchaseReportExcelExport(@ModelAttribute("command") @Valid clsExciseFLR4ReportBean objbean, HttpServletResponse resp,HttpServletRequest req)
		{
			...
			String sqlPurchase="SELECT a.strTpDate, c.strSupplierName,a.strTPNo,a.strInvoiceNo,d.strBrandName,e.strSizeName,b.intBottals "
							  +"FROM tbltphd a,tbltpdtl b,tblsuppliermaster c,tblbrandmaster d,tblsizemaster e "
							  +"WHERE a.strTPCode=b.strTPCode AND a.strSupplierCode=c.strSupplierCode AND b.strBrandCode=d.strBrandCode "
							  + "AND d.strSizeCode=e.strSizeCode AND a.strTpDate BETWEEN '"+fromDate+"' AND '"+toDate+"' AND a.strClientCode='"+clientCode+"' "
							  +"ORDER BY a.strTpDate,a.strSupplierCode,a.strInvoiceNo,a.strTPNo ";
