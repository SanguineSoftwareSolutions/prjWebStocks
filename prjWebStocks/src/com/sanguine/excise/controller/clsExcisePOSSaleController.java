...
public class clsExcisePOSSaleController{
	...
	public ModelAndView funOpenForm(Map<String, Object> model,HttpServletRequest request){
		...
		String sql_Licence="select strLicenceCode,strLicenceNo from tbllicencemaster where "
...
//Load Master Data On Form
	...
	public @ResponseBody List funLoadMasterData(HttpServletRequest request){
		...
		String sql="insert into tblexciseposlinkup (strPOSItemCode,strPOSItemName,strClientCode) "
				+ "(select distinct b.strPOSItemCode,b.strPOSItemName,b.strClientCode from tblexcisepossale b "
				+ " where b.strClientCode='"+clientCode+"' and b.strPOSItemCode not in (select c.strPOSItemCode "
				+ " from tblexciseposlinkup c where c.strClientCode='"+clientCode+"'))";
		...
		sql="select a.strPOSItemCode,a.strPOSItemName, ifnull(b.strBrandCode,'') as strBrandCode,"
				+ "ifnull(b.strBrandName,''),a.strBillNo,sum(a.intQuantity),a.dblRate,date(a.dteBillDate),a.intId,a.strPOSCode "
				+ "from tblexcisepossale a left outer join tblexciseposlinkup b on a.strPOSItemCode=b.strPOSItemCode "
				+ "left outer join tblbrandmaster c on a.strBrandCode=c.strBrandCode "
				+ "where a.strPOSCode='"+posCode+"' and date(a.dteBillDate) between '"+fromDate+"' and '"+toDate+"' "
				+ " group by a.strPOSItemCode order by strBrandCode";
						...
						if (obj.getStrBillNo().trim() != null) {
							if (!(obj.getStrBillNo().trim().length() >0)) {
								if (Long.valueOf(obj.getIntQuantity()) != null) {
									...
									String sql_isRecipe = "select a.strParentCode,a.strParentName from tblexciserecipermasterhd a "
											+ "where strParentCode='"+ obj.getStrBrandCode()+ "' and a.strClientCode='"+ clientCode + "'";
...
public Object funGenerateBill(clsExciseSaleBean objBean,clsExcisePOSSaleModel objModel,HttpServletRequest req){
		...
		String sql_BrandList="select a.strBrandCode,a.strSizeCode,a.strShortName,b.intQty,a.intPegSize,"
				+ " a.dblPegPrice,a.dblMRP,c.strSubCategoryName,c.intMaxSaleQty,d.intConversionFactor "
				+ " from tblbrandmaster a, tblsizemaster b,tblsubcategorymaster c, tblexciseposlinkup d"
				+ " where a.strBrandCode='"+objModel.getStrBrandCode()+"' and a.strSizeCode=b.strSizeCode "
				+ " and a.strBrandCode=d.strBrandCode and d.strClientCode='"+clientCode+"' "
				+ " and a.strSubCategoryCode=c.strSubCategoryCode and  a.strClientCode='"+tempBrandClientCode+"' "
				+ " and b.strClientCode='"+tempSizeClientCode+"' ORDER BY b.intQty DESC";			
		...
		String sql_OpData="select c.intOpBtls,c.intOpPeg,c.intOpML from tblbrandmaster a,tblopeningstock c "
							+ " where a.strBrandCode='"+objModel.getStrBrandCode()+"' and a.strBrandCode=c.strBrandCode  "
