...
public class clsCashMemoReportController {
	...
	public ModelAndView funOpenPurchaseAnylasisReport(
		...
		if ("2".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmExciseCashMemoReport_1", "command",
					new clsReportBean());
		} else {
			return new ModelAndView("frmExciseCashMemoReport", "command",
					new clsReportBean());
		}
	...
	private void funCallPurchaseAnylasisReportReportt( @ModelAttribute("command") clsReportBean objBean, HttpServletResponse resp, HttpServletRequest req) {
		...
		try {
				...
				String strfindFirstNo = "SELECT MIN(intBillNo) FROM tblexcisesaledata "
