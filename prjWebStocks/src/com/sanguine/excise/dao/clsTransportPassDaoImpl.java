...
public class clsTransportPassDaoImpl implements clsTransportPassDao{
	...
	public List funGetObject(String tpCode,String clientCode){
		...
		try{
			Session currentSession=exciseSessionFactory.getCurrentSession();
			Query query = currentSession.createQuery("from clsTransportPassModel a,clsExciseSupplierMasterModel b, clsExciseLicenceMasterModel c "
					+ "where a.strTPCode= :strTPCode and a.strSupplierCode=b.strSupplierCode and a.strLicenceCode=c.strLicenceCode and a.strClientCode= :clientCode");
	...
	public List funGetTPDtlList(String tpCode,	String clientCode) {
		...
		try{
			Session currentSession=exciseSessionFactory.getCurrentSession();
			Query query = currentSession.createQuery("from clsTransportPassDtlModel a,clsBrandMasterModel b, clsSizeMasterModel c  "
					+ "where strTPCode= :Code and a.strBrandCode= b.strBrandCode and b.strSizeCode=c.strSizeCode and a.strClientCode= :clientCode");
	...
	public boolean funDeleteHd(String tpCode, String clientCode) {
		...
		try{
			Session currentSession=exciseSessionFactory.getCurrentSession();
			Query query = currentSession.createQuery("delete clsTransportPassModel where strTPCode = :tpCode "
	...
	public boolean funDeleteDtl(String tpCode, String clientCode) {
		...
		try{
			Session currentSession=exciseSessionFactory.getCurrentSession();
			Query query = currentSession.createQuery("delete clsTransportPassDtlModel where strTPCode = :tpCode "
					+ "and strClientCode=:clientCode");
			query.setParameter("tpCode", tpCode);
			query.setParameter("clientCode", clientCode);
			query.executeUpdate();	
	...
	public List funGetTpNOObject(String tpNo,String clientCode)
	{
		...
		try{
			Session currentSession=exciseSessionFactory.getCurrentSession();
			Query query = currentSession.createQuery("from clsTransportPassModel a "
					+ "where a.strTPNo= :strTPNo and a.strClientCode= :clientCode ");
