...
public class clsExciseBillGenerateDaoImpl implements clsExciseBillGenerateDao{
	...
	public List funGetSalesDtlList(long intId, String clientCode) {
		...
		try{
			...
			String sql_Qry="from clsExciseManualSaleDtlModel a, clsBrandMasterModel b, clsSizeMasterModel c "
					+ " where a.intSalesHd='"+intId+"' and b.strSizeCode=c.strSizeCode and "
					+ " a.strBrandCode=b.strBrandCode and a.strClientCode= '"+clientCode+"' "
					+ " order by b.strSubCategoryCode,c.intQty desc ";
	...
	public void funDeleteSaleData(String strFromDate, String strToDate,String clientCode,String licenceCode){
		try{
			Session currentSession=exciseSessionFactory.getCurrentSession();
			Query query = currentSession.createSQLQuery("DELETE FROM tblexcisesaledata WHERE "
					+ " date(dteBillDate) >= '"+strFromDate+"' AND date(dteBillDate) <= '"+strToDate+"' and strClientCode='"+clientCode+"' and strLicenceCode='"+licenceCode+"' and strSourceEntry='Manual' " );
			query.executeUpdate();
