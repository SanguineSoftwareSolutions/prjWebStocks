...
public class clsExciseStkPostingDaoImpl implements clsExciseStkPostingDao
{
	...
	public List funGetObject(String strPSPCode,String clientCode)
	{
		...
		try{
			Session currentSession=exciseSessionFactory.getCurrentSession();
			Query query = currentSession.createQuery("from clsExciseStkPostingHdModel a,clsExciseLicenceMasterModel b "
					+ " where a.strPSPCode= :strPSPCode and a.strLicenceCode=b.strLicenceCode "
					+ " and a.strClientCode= :clientCode and b.strClientCode= :licClientCode");
	...
	{ 
		...
		try{
			Session currentSession=exciseSessionFactory.getCurrentSession();
			Query query = currentSession.createQuery("from clsExciseStkPostingDtlModel a,clsBrandMasterModel b, clsSizeMasterModel c "
					+ " where a.strPSPCode = :strPSPCode and a.strBrandCode=b.strBrandCode "
					+ " and b.strSizeCode = c.strSizeCode and a.strClientCode= :clientCode"
