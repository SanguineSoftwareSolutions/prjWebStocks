...
public class clsOpeningStockDaoImpl implements clsOpeningStockDao{
	...
	public clsOpeningStockModel funGetExBrandOpMaster(Long intId,String strClientCode){
		...
		try{
			List ls=null;
			Session currentSession=exciseSessionFactory.getCurrentSession();
			Query query = currentSession.createQuery("from clsOpeningStockModel a "
					+ "where a.intId= :intId and a.strClientCode= :strClientCode");
	...
	public List funGetMasterObject(Long intId,String strClientCode ){
		...
		try{
			Session currentSession=exciseSessionFactory.getCurrentSession();
			Query query = currentSession.createQuery("from clsOpeningStockModel a,clsBrandMasterModel b "
					+ "where a.intId= :intId and a.strBrandCode=b.strBrandCode and a.strClientCode= :strClientCode ");
			query.setParameter("intId", intId);
			query.setParameter("strClientCode", strClientCode);
	...
	public Boolean funDeleteBrandOpening(Long intId, String clientCode) {
		...
		try{
			Session currentSession=exciseSessionFactory.getCurrentSession();
			Query query = currentSession.createQuery("delete clsOpeningStockModel where intId = :intId "
	...
	public clsOpeningStockModel funGetOpenedEXBrandMaster(String brandCode,String licenceCode,String clientCode) {
		...
		try{
			...
			List ls=null;
			Session currentSession=exciseSessionFactory.getCurrentSession();
			Query query = currentSession.createQuery("from clsOpeningStockModel where strBrandCode = :brandCode "
					+ "and strClientCode=:clientCode and strLicenceCode=:licenceCode");
