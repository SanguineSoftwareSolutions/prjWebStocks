...
public class clsExciseRecipeMasterDaoImpl implements clsExciseRecipeMasterDao{
	...
	public List funGetObject(String RecipeCode,String clientCode){
		...
		try{
			Session currentSession=exciseSessionFactory.getCurrentSession();
//			Query query = currentSession.createQuery("from clsExciseRecipeMasterHdModel a, clsBrandMasterModel b  "
//					+ "where strRecipeCode= :RecipeCode and a.strParentCode=b.strBrandCode and a.strClientCode= :clientCode");
			
			Query query = currentSession.createQuery("from clsExciseRecipeMasterHdModel a "
					+ "where a.strRecipeCode= :RecipeCode and a.strClientCode= :clientCode");
	...
	public List<clsExciseRecipeMasterDtlModel> funGetRecipeDtlList(String RecipeCode,	String clientCode) {
		...
		try{
			Session currentSession=exciseSessionFactory.getCurrentSession();
			Query query = currentSession.createQuery("from clsExciseRecipeMasterDtlModel a, clsBrandMasterModel b "
					+ "where a.strRecipeCode= :RecipeCode and a.strBrandCode=b.strBrandCode and a.strClientCode= :clientCode");
			query.setParameter("RecipeCode", RecipeCode);
			query.setParameter("clientCode", clientCode);
			ls = query.list();
	...
	public void funDeleteDtl(String RecipeCode, String clientCode) {
		try{
			Session currentSession=exciseSessionFactory.getCurrentSession();
			Query query = currentSession.createQuery("delete clsExciseRecipeMasterDtlModel where strRecipeCode = :RecipeCode "
