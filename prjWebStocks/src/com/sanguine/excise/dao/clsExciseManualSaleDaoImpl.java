...
public class clsExciseManualSaleDaoImpl implements clsExciseManualSaleDao{
	...
	public List funGetObject(long intId,String clientCode){
		...
		try{
			Session currentSession=exciseSessionFactory.getCurrentSession();
			Query query = currentSession.createQuery("from clsExciseManualSaleHdModel a, clsExciseLicenceMasterModel b"
					+ " where a.intId= :intId and a.strLicenceCode=b.strLicenceCode and b.strClientCode= :clientCode and a.strClientCode= :clientCode");
	...
	public List funGetSalesDtlList(long intId, String clientCode) {
		...
		try{
			Session currentSession=exciseSessionFactory.getCurrentSession();
			Query query = currentSession.createQuery("from clsExciseManualSaleDtlModel a, clsBrandMasterModel b "
					+ "where a.intSalesHd= :intSalesHd and a.strBrandCode=b.strBrandCode and a.strClientCode= :clientCode");
	...
	public boolean funDeleteHd(long intId, String clientCode) {
		...
		try{
			Session currentSession=exciseSessionFactory.getCurrentSession();
			Query query = currentSession.createQuery("delete clsExciseManualSaleHdModel where intId = :intId "
	...
	public boolean funDeleteDtl(long intId, String clientCode) {
		...
		try{
			Session currentSession=exciseSessionFactory.getCurrentSession();
			Query query = currentSession.createQuery("delete clsExciseManualSaleDtlModel where intSalesHd = :intSalesHd  "
					+ "and strClientCode=:clientCode");
			query.setParameter("intSalesHd", intId);
			query.setParameter("clientCode", clientCode);
	...
	public boolean funDeleteSaleData(long intId, String clientCode) {
		...
		try{
			Session currentSession=exciseSessionFactory.getCurrentSession();
			Query query = currentSession.createQuery("delete clsExciseSaleModel where strSalesCode = :strSalesCode "
					+ "and strClientCode=:clientCode");
