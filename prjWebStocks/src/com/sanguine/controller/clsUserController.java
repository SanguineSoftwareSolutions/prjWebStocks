...
//								String alphYear=objGlobalFun.funGetAlphabet(yearcalNo);
//								String alphmonth=objGlobalFun.funGetAlphabet((month));
//								String alphday=objGlobalFun.funGetAlphabet((day%26));
							//	String finalPassword=String.valueOf(Password)+alphYear+alphmonth+alphday;
	...
	//	req.getSession().setAttribute("PropertyError","");
		...
		//req.getSession().setAttribute("warDate",funWarFileDate());
...
//                    jObjLic.put("CheckLicence", arrObj);
	...
	private ModelAndView frmWebBookModuleSelection(HttpServletRequest req){
        ...
        req.getSession().setAttribute("moduleMap",moduleMap);
		return new ModelAndView("frmWebBookModuleSelection");
	}
...
//				frmWebBookModuleSelection(req);
	...
	{	
		...
		String sql_UserLog="select max(strLoggedInLocation),max(strLoggedInProperty) "
			+ "from tbluserlogs where strUserCode='"+usercode+"' and strClientCode='"+clientCode+"' "
			+ "and dteLoginDate= (select max(dteLoginDate) from tbluserlogs)";
	...
	/*@SuppressWarnings({ "unchecked", "rawtypes" })
	@RequestMapping(value="loadLoactionComboPropertyWise", method = RequestMethod.GET)
	public @ResponseBody Map<String,String> funLoadLocComboPropertyWise(@RequestParam("code") String propertyCode,HttpServletRequest req){
		String clientCode=req.getSession().getAttribute("clientCode").toString();
		String usercode=req.getSession().getAttribute("usercode").toString();
		HashMap<String,String> locMap=(HashMap)objLocationMasterService.funGetLocMapPropertyWise(propertyCode,clientCode,usercode);
	...
	public @ResponseBody Map<String,String> funLoadLocComboPropertyWise(@RequestParam("code") String propertyCode,HttpServletRequest req){
		String clientCode=req.getSession().getAttribute("clientCode").toString();
		String usercode=req.getSession().getAttribute("usercode").toString();
		HashMap<String,String> locMap=(HashMap)objUserMasterService.funGetLocMapPropertyNUserWise(propertyCode,clientCode,usercode,req);
		locMap=clsGlobalFunctions.funSortByValues(locMap);
		return locMap;
...
//		{
//			String sql = "select tmeCheckInTime,tmeCheckOutTime from tblpropertysetup "
//				+ " where strPropertyCode='" + propBean.getStrPropertyCode() + "' and strClientCode='"+clientCode+"'";
...
//			else
//			{
//				req.getSession().setAttribute("PMSDate", objGlobalFun.funGetCurrentDate("yyyy-MM-dd"));
...
//	}
	...
	try
	{
		...
		if(req.getSession().getAttribute("selectedModuleName").equals("7-WebPOS"))
		{
//			List<clsUserDesktopUtil> webPOSDesktop = funGetPOSMenuMap("super", "117.001", "M", "Super", "P01");
//			req.getSession().setAttribute("desktop",webPOSDesktop);
		...
		{
//			List<clsUserDesktopUtil> webPOSDesktop = funGetPOSMenuMap("super", "117.001", "M", "Super", "P01");
//			req.getSession().setAttribute("treeMap",new HashMap<String,Object>());
		}
		else
		{
			Map<String,Object> tr = funGetTreeMap(req);
...
//			String query="select distinct a.strRootNode from tbltreemast a,tbluserdtl b where a.strFormName=b.strFormName "
//				+ "and b.strUserCode='"+userCode+"' and b.strClientCode='"+clientCode+"' and a.strModule='"+moduleCode+"' and strType<>'O' order by a.intRootIndex";
			...
			String query="select * from (select distinct a.strRootNode,a.intRootIndex "
					+ " from tbltreemast a,tbluserdtl b where a.strFormName=b.strFormName and b.strUserCode='"
					+userCode+"' and b.strClientCode='"+clientCode+"' and a.strModule='"+moduleCode+"' "
					+ " and strType<>'O' UNION select distinct x.strRootNode,x.intRootIndex "
					+ " from tbltreemast x where x.strFormName IN(select distinct p.strRootNode "
					+ " from tbltreemast p,tbluserdtl q where p.strFormName=q.strFormName "
					+ " and q.strUserCode='"+userCode+"' and q.strClientCode='"+clientCode+"' "
					+ " and p.strModule='"+moduleCode+"' and strType<>'O') and x.strModule='"+moduleCode+"') "
					+ " as s ORDER BY s.intRootIndex";
		...
		if("YES".equalsIgnoreCase(isSuperUser)){
			...
			String query="select distinct a.strFormName,a.intRootIndex from tbltreemast a,tbluserdtl b where "
				+ "b.strUserCode='"+userCode+"' and b.strClientCode='"+clientCode+"' and a.strModule='"+moduleCode+"' and strType='O' order by a.intRootIndex";
			//childNodeList=objGlobalService.funGetList(query);
			List childNode =objGlobalService.funGetList(query);
			childNodeList=new ArrayList();
			for(int i=0;i<childNode.size();i++)
...
/*	private String funWarFileDate()
	{
		...
		String filepath =System.getProperty("user.dir") +"\\webapps\\prjWebStocks.war";
		System.out.println( filepath+"\\prjWebStocks.war");
...
//	@SuppressWarnings("unused")
//	@RequestMapping(value = "/frmWebPOSPOSSelection", method = RequestMethod.GET)
...
//		try {
//			webPOSDesktop = funGetPOSMenuMap("super", "117.001", "M", "Super", strPOSCode);
//		} catch (Exception e) {
//			// TODO Auto-generated catch block
//			e.printStackTrace();
//		}
...
//				jArr.add(obj.getStrFormName());
	...
	/*@SuppressWarnings("unused")
	@RequestMapping(value = "/frmShowTodayCheckOutRooms", method = RequestMethod.GET)
	public ModelAndView frmShowTodayCheckOutRooms(HttpServletRequest req,Map<String,Object> model)
	{
		String urlHits="1";
		try
		{
		...
		String clientCode = req.getSession().getAttribute("clientCode").toString();
		String sql=" select c.strRoomDesc,d.strRoomTypeDesc "
				...
				+ " and date(a.dteDepartureDate) = '"+objGlobalFun.funGetCurrentDate("yyyy-MM-dd")+"'" ;
//				+ " group by a.strRoomNo " ;
		List listCheckout=objGlobalService.funGetListModuleWise(sql, "sql");
		if(listCheckout!=null)
		{
			model.put("listCheckout", listCheckout);
		}
		...
		{			
			return new ModelAndView("frmShowTodayCheckOutRooms","command",new clsGuestMasterBean());
		}
		else
		{
			return new ModelAndView("frmShowTodayCheckOutRooms_1","command",new clsGuestMasterBean());
		}		
	...
	public @ResponseBody List frmShowTodayCheckOutRooms(HttpServletRequest req,Map<String,Object> model)
	{
		...
		String clientCode = req.getSession().getAttribute("clientCode").toString();
		String sql=" select c.strRoomDesc,d.strRoomTypeDesc,CONCAT(e.strFirstName,' ',e.strMiddleName,' ',e.strLastName) "
				+ " from tblcheckinhd a,tblcheckindtl b,tblroom c,tblroomtypemaster d,tblguestmaster e  "
				+ " where a.strCheckInNo=b.strCheckInNo  and b.strRoomNo = c.strRoomCode "
				+ " and c.strRoomTypeCode=d.strRoomTypeCode and a.strClientCode=b.strClientCode and b.strGuestCode=e.strGuestCode  "
				+ " and b.strClientCode = c.strClientCode and c.strClientCode = a.strClientCode "
				+ " and a.strClientCode='"+clientCode+"' "
				+ " and date(a.dteDepartureDate) = '"+objGlobalFun.funGetCurrentDate("yyyy-MM-dd")+"'" ;
//				+ " group by a.strRoomNo " ;
	...
	private List<clsUserDesktopUtil> funGetPOSMenuMap(String userCode,String clientCode,String moduleType,String superUser,String POSCode) throws Exception 
	{
		...
		String posUrl = 	"http://localhost:8080/prjSanguineWebService/APOSIntegration/funGetMainMenu"
