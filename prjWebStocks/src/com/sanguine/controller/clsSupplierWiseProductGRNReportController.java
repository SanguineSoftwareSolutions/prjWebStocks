...
public class clsSupplierWiseProductGRNReportController {
	...
	private void funCallProductWiseReport(@ModelAttribute("command") clsProductWiseGRNReportBean objBean, HttpServletResponse resp, HttpServletRequest req)
    {
  	...
  	for (int i = 0; i < tempPCode.length; i++) {
  	 ...
  	 String sqlQuery = "select c.strSGName,e.strPName,d.strProdName,sum(b.dblQty) ,sum(b.dblTotalPrice),d.strUOM ,f.dblLastCost  "
  	 		+ " from tblgrnhd a,tblgrndtl b ,tblsubgroupmaster c,tblproductmaster d,tblpartymaster e ,tblprodsuppmaster f " 
  			 		  +" where a.strGRNCode=b.strGRNCode and b.strProdCode=d.strProdCode "
  			 		  +" and d.strSGCode=c.strSGCode and a.strSuppCode =e.strPCode and a.strSuppCode=f.strSuppCode and b.strProdCode=f.strProdCode "
  			 		  +" and a.dtGRNDate between '"+dteFromDate+"' and '"+dteToDate+"'  "
  	 				+ " and ("+pCode+")  "+  " and ("+strSGCode+" ) ";
