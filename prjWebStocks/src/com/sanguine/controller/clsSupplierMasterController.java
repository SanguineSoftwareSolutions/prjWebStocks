...
public class clsSupplierMasterController {
	...
	//Save or Update group master function to save or update record of group master into database and also validates all fields of form.
		...
		{
			String clientCode=req.getSession().getAttribute("clientCode").toString();
			String sql="select a.strTaxCode,b.strTaxDesc from tblpartytaxdtl a,tbltaxhd b "
					+ "where a.strTaxCode=b.strTaxCode and a.strPCode='"+PCode+"'";
		...
		private clsSupplierMasterModel funPrepareModel(clsSupplierMasterBean objBean,String userCode,String clientCode)
		{
			...
			//objModel.setStrPType(objGlobal.funIfNull(objBean.getStrPType(), " ",objBean.getStrPType()));
			//objModel.setArrListPartyTaxDtlModel(objBean.getListclsPartyTaxIndicatorDtlModel());
		...
		model.put("urlHits", urlHits);
			...
			if ("2".equalsIgnoreCase(urlHits)) {
				return new ModelAndView("frmProdWiseSuppRateHis_1", "command",
						objRptBean);
			} else {
				return new ModelAndView("frmProdWiseSuppRateHis", "command",
						objRptBean);
			}
		...
		private void funCallReport(clsReportBean objBean,HttpServletResponse resp,HttpServletRequest req)
		{
			try {
				...
				String reportName=servletContext.getRealPath("/WEB-INF/reports/rptProdWiseSuppHis.jrxml"); 
				String imagePath=servletContext.getRealPath("/resources/images/company_Logo.png");
				
				String sqlHDQuery="select 1 from dual";
				
				String sqlDtlQuery="select b.strProdCode, d.strProdName, c.strPName, a.strAgainst , a.strPONo, date(a.dtGRNDate) as dtGRNDate, "
						...
						+" and b.strProdCode = d.strProdCode and a.dtGRNDate >= '"+frmDate+"' and a.dtGRNDate <= '"+ToDate+"' "
				...
				sqlDtlQuery=sqlDtlQuery+" order by a.dtGRNDate, a.strGRNCode ";
...
//					objModel.setStrSupplierNo(objBean.getStrSupplierNo());
		...
		private clsSundaryCreditorMasterModel funPrepareDebtorModel(clsSupplierMasterBean objBean,String userCode,String clientCode,String propertyCode)
		{
			...
			{
				clsSundaryCreditorMasterModel objModel1=objSundryCreditorMasterService.funGetSundryCreditorMaster(objBean.getStrDebtorCode().trim(), clientCode);
				if(null == objModel1)
				{
					lastNo=objGlobalFunctionsService.funGetLastNoModuleWise("tblsundarycreditormaster","CreditorMaster","intGId", clientCode,"5-WebBook");
					String debtorCode ="C"+String.format("%08d", lastNo);
