...
//            String sql=" select a.strGRNCode,a.strSuppCode,c.strPName,a.strAgainst,a.strBillNo,Date(a.dtGRNDate) , a.dblTotal,d.strTallyCode  "
...
//            		+ " group by a.strGRNCode ";
            ...
            String sql=" select a.strGRNCode,a.strSuppCode,c.strPName,a.strAgainst,a.strBillNo,Date(a.dtGRNDate) , a.dblTotal,ifnull(d.strTallyCode,'')   "
            		+ " from tblgrnhd a "
	 ...
	 private void funSupplierLedgerXMLFile(clsReportBean objBean,String clientCode )
	 {
        ...
        try {
            ...
            envelopeElement.appendChild(getHeader(doc, "1", "Import", "Data", "All Masters")); // for Header
            
            Element bodyElement = doc.createElement("BODY");
            
            envelopeElement.appendChild(bodyElement);
            
            bodyElement.appendChild(getSuppLedgerDesc(doc, "")); //For Desc body
            ...
            String sql= "  select b.strSuppCode,a.strPName,c.strTallyCode from tblpartymaster a ,tblgrnhd b,tbltallylinkup c   "
            		+ " where  a.strPCode=b.strSuppCode and  a.strPType='supp' and c.strGroupCode=b.strSuppCode "
            		+ " and date(b.dtGRNDate)  between  '"+objBean.getDteFromDate()+"' and '"+objBean.getDteToDate()+"' "
			...
			String sqlPer=" select b.strTaxDesc from tblgrnhd a , tblgrntaxdtl b ,tbltaxhd c ,tblpartymaster d "
					+ " where a.strGRNCode=b.strGRNCode and b.strTaxCode=c.strTaxCode and c.strTaxOnSP='Purchase' and a.strSuppCode=d.strPCode  "
					+ " and date(a.dtGRNDate) between '"+objBean.getDteFromDate()+"' and '"+objBean.getDteToDate()+"' "
		...
		private  Node getData(Document doc, String id, String date, String billNo, String typeSP,
	            String vCount,String strGRNNo,String strSuppName,String isDeempositive,String dblGrandTotal,clsReportBean objBean,String clientCode) {
...
//	        employee.setAttribute("id", id);
	     ...
	     // for Bill 
	        ...
	        String sql="  select b.strTaxDesc,b.strTaxableAmt "
	        		+ "  from tblgrnhd a ,tblgrntaxdtl b "
	        		+ "  where a.strGRNCode=b.strGRNCode and "
	        		+ " a.strGRNCode='"+strGRNNo+"' "
	        		+ " and date(a.dtGRNDate)  between  '"+objBean.getDteFromDate()+"' and '"+objBean.getDteToDate()+"' "
	        		+ "  and a.strClientCode='"+clientCode+"' and b.strClientCode='"+clientCode+"'  ";
		...
		private  Node getSaleData(Document doc, String id, String date, String invCode, String typeSP,
	            String vCount,String invNo,String strCustName,String isDeempositive,String dblGrandTotal,clsReportBean objBean,String clientCode) {
	        Element voucher = doc.createElement("VOUCHER");
...
//	        employee.setAttribute("id", id);
		...
		try{
	            ...
	            String sql="select a.strInvCode,b.strPName,a.strCustCode from tblinvoicehd a,tblpartymaster b  "
	            		+" where a.strCustCode=b.strPCode and date(a.dteInvDate) between '"+objBean.getDteFromDate()+"' and '"+objBean.getDteToDate()+"' "
	            		+ " and a.strClientCode='"+clientCode+"'  and b.strClientCode='"+clientCode+"' "
	            		+ " ORDER by b.strPName  ";
				...
				String sqlTax="select a.strTaxDesc,c.dblPercent  from tblinvtaxdtl a ,tblinvoicehd b,tbltaxhd c "
							 +" where b.strInvCode='"+invCode+"' and a.strInvCode=b.strInvCode and c.strTaxOnSP='Sales' " 
						     +" and a.strTaxCode=c.strTaxCode and date(b.dteInvDate) between '"+objBean.getDteFromDate()+"' and '"+objBean.getDteToDate()+"'  "
						     + " and a.strClientCode='"+clientCode+"'  and b.strClientCode='"+clientCode+"' ";
		 ...
		 private void funPurchaseXMLFileFormate1(clsReportBean objBean,String clientCode,HttpServletRequest req )
		 {
	        ...
	        try {
	          ...
	          //  clsCompanyMasterModel objCompModel = objSetupMasterService.funGetObject(clientCode);
	            importDataElement.appendChild(getRequestDescFormate1(doc,"Vouchers",companyName)); // Company name accoring to tally company
	            ...
	            String sql=" select a.strGRNCode,a.strSuppCode,c.strPName,a.strAgainst,a.strBillNo,Date(a.dtGRNDate) , "
	            		+ " a.dblTotal,ifnull(d.strTallyCode,''),a.strUserModified ,DATE_FORMAT(a.dtDueDate,'%d-%b-%Y') ,DATE_FORMAT(a.dtGRNDate,'%b')   "
	            		+ " from tblgrnhd a "
	            		+ " left outer join tblgrntaxdtl b on a.strGRNCode=b.strGRNCode  and b.strClientCode='"+clientCode+"' "
	            		+ " left outer join tblpartymaster c on a.strSuppCode=c.strPCode and c.strClientCode='"+clientCode+"' "
	            		+ " left outer join tbltallylinkup d on d.strGroupCode=a.strSuppCode "
	            		+ " where  date(a.dtGRNDate) between  '"+objBean.getDteFromDate()+"' and '"+objBean.getDteToDate()+"'  "
	            		+ " and a.strClientCode='"+clientCode+"' and a.strGRNCode like '"+strPropCode+"'  ";
...
//				}
		...
		private  Node getDataFormate1(Document doc, String id, String date, String billNo, String typeSP,
	            String vCount,String strGRNNo,String strSuppName,String isDeempositive,String dblGrandTotal,clsReportBean objBean,
	            String clientCode,String companyName,String userName,String billDateddMMyyyy,String billMonth) {
	        ...
	        voucher.appendChild(getVoucherElements(doc, voucher, "VOUCHERNUMBER", strGRNNo)); // TAlly system gentarted no
		   ...
		   String subGroupSql = " select b.strTallyCode,b.strGroupName ,sum(a.dblTotalPrice) "
		   		+ " from tblgrndtl a , tbltallylinkup b ,tblproductmaster c "
		   		+ " where a.strProdCode=c.strProdCode  and b.strGroupCode=c.strSGCode  "
		   		+ " and a.strGRNCode = '"+strGRNNo+"'  "
		   		+ " and a.strClientCode='"+clientCode+"' "
		   		+ " and b.strClientCode='"+clientCode+"' "
		   		+ " and c.strClientCode='"+clientCode+"' "
		...
		String sql =" select c.strTallyCode,b.strTaxDesc,b.strTaxAmt from tblgrnhd a,tblgrntaxdtl b ,tbltallylinkup c "
				+ " where  a.strGRNCode=b.strGRNCode and  b.strTaxCode=c.strGroupCode "
				+ " and a.strGRNCode='"+strGRNNo+"' and a.strClientCode='"+clientCode+"' "
