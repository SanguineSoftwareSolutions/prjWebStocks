...
//			            	}
		...
		public ModelAndView funOpenStkAdjSlipForm(
			...
			if ("2".equalsIgnoreCase(urlHits)) {
				return new ModelAndView("frmStockAdjustmentSlip_1", "command",
						new clsReportBean());
			} else {
				return new ModelAndView("frmStockAdjustmentSlip", "command",
						new clsReportBean());
			}
		...
		public void funCallReportBeanWise(String stockAdjCode,String type,HttpServletResponse resp,HttpServletRequest req)
		{
			try {
				...
				String sqlDtlQuery="select a.strProdCode,a.strRemark,b.strProdName, "
						+ "b.strProdType,sum(a.dblQty),b.strUOM,a.strType,a.dblRate,a.strDisplayQty,a.dblPrice from tblstockadjustmentdtl a, "
						+ "tblproductmaster b where a.strProdCode=b.strProdCode and  a.strClientCode='"+clientCode+"' "
						+ " and b.strClientCode='"+clientCode+"' and a.strSACode='"+stockAdjCode+"' "
		...
		public void funCallReportRecipeWise(String stockAdjCode,String type,HttpServletResponse resp,HttpServletRequest req)
		{
			...
			//	 List<clsStkAdjustmentDtlModel> listDateWiseDtl  = objStkAdjService.funGetDatewiseDtlList("2017-05-01", "2017-05-31", clientCode);
				...
				for(String strProdsINOUT :hsRemarkProd)
				{
					...
					String sqlRecipeQuery=" select b.strChildCode,b.dblQty,b.strUOM from tblbommasterhd a,tblbommasterdtl b where a.strBOMCode=b.strBOMCode "
							+ " and a.strParentCode='"+strProd+"' ";
							...
							String strRecChild =  objBOM[0].toString();
					 		 {
					 			 ...
					 			 String sqlChildProdQuery = " select sum(a.dblQty), sum(a.dblPrice) from tblstockadjustmentdtl a "
					 					+ " where a.strProdCode='"+strRecChild+"' "
					 					+ " and a.strRemark like '"+parentProd+"' and a.strSACode='"+stockAdjCode+"' and a.strType='"+inOut+"' ";
...
//	  	            hm.put("dtToSADate", objGlobal.funGetDate("dd-MM-yyyy", dtSADate));
	  	            hm.put("strNarration", strNarration);
			...
			public ModelAndView funOpenStkAdjMonthlyForm(
					Map<String, Object> model,
					HttpServletRequest request)
		    	{
				String urlHits = "1";
				...
				if ("2".equalsIgnoreCase(urlHits)) {
					return new ModelAndView("frmStockAdjustmentMonthly_1", "command",
							new clsReportBean());
				} else {
					return new ModelAndView("frmStockAdjustmentMonthly", "command",
							new clsReportBean());
				}
				...
				try
				{
					...
					if(objBean.getStrReportType().equals("Physical Stock") || objBean.getStrReportType().equals("Manually"))
					{
						...
						if(objBean.getStrReportType().equals("Physical Stock"))
						{
							...
							sqlDtlQuery=" SELECT b.strProdCode,b.strRemark,c.strProdName, c.strProdType, "
									+ " SUM(b.dblQty),c.strUOM,b.strType,  b.dblRate,b.strDisplayQty,b.dblPrice "
									+ " FROM tblstockadjustmenthd a,tblstockadjustmentdtl b, tblproductmaster c "
									+ " WHERE a.strSACode=b.strSACode and Date(a.dtSADate) between '"+dtFDate+"' and '"+dtTDate+"' "
									+ " and b.strProdCode=c.strProdCode AND b.strClientCode='"+clientCode+"' "
									+ " and b.strRemark like '%Physical Stock%' AND b.strClientCode='"+clientCode+"'  "; 
						...
						if(objBean.getStrReportType().equals("Manually"))
						{
							...
							sqlDtlQuery=" SELECT b.strProdCode,b.strRemark,c.strProdName, c.strProdType,  SUM(b.dblQty), "
									+ " c.strUOM,b.strType, b.dblRate,b.strDisplayQty,b.dblPrice "
									+ " FROM tblstockadjustmenthd a,tblstockadjustmentdtl b, tblproductmaster c "
									+ " WHERE a.strSACode=b.strSACode and "
									+ " Date(a.dtSADate) between '"+dtFDate+"' and '"+dtTDate+"' "
									+ " and b.strProdCode=c.strProdCode AND b.strClientCode='"+clientCode+"' "
									+ " AND b.strClientCode='"+clientCode+"' "
									+ " and a.strNarration not in (  select ph1.strNarration   from tblstockadjustmenthd ph1 "
																+ " where Date(ph1.dtSADate) between '"+dtFDate+"' and '"+dtTDate+"'  "
																+ " and (ph1.strNarration like '%Physical Stock%' or ph1.strNarration like '%Sale Data%' "
																+ " or ph1.strNarration like '%Sales Data%' AND ph1.strClientCode='"+clientCode+"' )) "
																+ " GROUP BY b.strProdCode,b.strType "
																+ " ORDER BY a.strSACode; ";
						}
							 List listProdDtl = objGlobalFunctionsService.funGetDataList(sqlDtlQuery,"sql");
							 List<clsStkAdjustmentDtlModel> listDtlBean= new ArrayList<clsStkAdjustmentDtlModel>();
							  for (int j = 0; j < listProdDtl.size(); j++)
					...
					if(objBean.getStrReportType().equals("POS"))
					{
						...
						String sqlHDQuery=" select a.strNarration,a.strAuthorise from tblstockadjustmenthd a "
								+ " where a.dtSADate between '"+dtFDate+"' and '"+dtTDate+"' "
								+ " and LENGTH(a.strNarration)>0 "
								+ " and a.strNarration like '%Sales Data%' "
								+ " and a.strClientCode='"+clientCode+"' limit 1;";
						...
						// List<clsStkAdjustmentDtlModel> listDtl  = objStkAdjService.funGetDatewiseDtlList(dtFDate, dtTDate, clientCode);
					...
					//	List<clsStkAdjustmentDtlModel> listDtl  = objStkAdjService.funGetDtlList(stockAdjCode, clientCode);
						...
						for(clsStkAdjustmentDtlModel objAstkAdj: listGroupedDtl)
						{
							String remark = objAstkAdj.getStrRemark();
							String[] spRemark = remark.split(":");
							String strCheckkey=objAstkAdj.getStrWSLinkedProdCode()+"#"+objAstkAdj.getStrSACode()+"#"+spRemark[7];
							
							if(hmRemarkProd.containsKey(spRemark[3]+"#"+spRemark[7]))
							{
								clshmReportBean objBeanHm =hmRemarkProd.get(spRemark[3]+"#"+spRemark[7]);
								String mapChekKey=objBeanHm.getStrPraentProdCode()+"#"+objBeanHm.getStrSACode()+"#"+spRemark[7];
								if(!strCheckkey.equals(mapChekKey))
								{
									/*if(objBeanHm.getStrPraentProdCode().equalsIgnoreCase("P0000776"))
									{
										System.out.println("");
									}*/
									
									objBeanHm.setDblQty(objBeanHm.getDblQty()+Double.parseDouble(spRemark[5].trim()));
									objBeanHm.setStrSACode(objBeanHm.getStrSACode());
									hmRemarkProd.put(objBeanHm.getStrPraentProdCode()+"#"+spRemark[7], objBeanHm);
								}
							}else
							{
								clshmReportBean objBeanHm = new clshmReportBean();
								objBeanHm.setStrPraentProdCode(spRemark[3]);
								objBeanHm.setStrBOMCode(spRemark[1]);
								objBeanHm.setStrItemName(spRemark[7]);
								objBeanHm.setStrInOut("Out");
								objBeanHm.setDblprice(objAstkAdj.getDblPrice());
								objBeanHm.setDblRate(objAstkAdj.getDblRate());
								objBeanHm.setDblQty(Double.parseDouble(spRemark[5]));
						...
						//	String[] spCodeInOutQty =strProdsINOUT.split("#");
							...
							{
								String sqlRecipeQuery=" select b.strChildCode,b.dblQty,b.strUOM from tblbommasterhd a,tblbommasterdtl b,tblproductmaster c "
										+ " where a.strBOMCode=b.strBOMCode and a.strParentCode=c.strProdCode "
										+ " and a.strParentCode='"+strProd+"'and c.strProdName='"+itemName+"' ";
										...
										String strRecChild =  objBOM[0].toString();
								 		 {
								 			...
								 			String positemName="%"+itemName+"%";
								 			 String sqlChildProdQuery = " select sum(a.dblQty), sum(a.dblPrice) "
								 			 		+ " from tblstockadjustmentdtl a,tblstockadjustmenthd b "
								 			 		+ " where a.strSACode=b.strSACode and b.dtSADate between '"+dtFDate+"' and '"+dtTDate+"' "
								 			 		+ "  and a.strProdCode='"+strRecChild+"' and b.strNarration like '%Sales Data%' "
								 			 		+ "  and a.strRemark like '"+positemName+"' and a.strWSLinkedProdCode='"+parentProd+"' "
								 			 		+ " and a.strType='"+inOut+"'  ";
								 			if(strProd.equalsIgnoreCase("P0000776"))
											{
												System.out.println("");
