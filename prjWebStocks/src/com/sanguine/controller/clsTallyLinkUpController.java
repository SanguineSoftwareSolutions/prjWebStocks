...
//			{
				 sql= " select a.strPCode,a.strPName,ifnull(b.strGDes,''),ifnull(b.strTallyCode,'') "
				 		+ " from tblpartymaster a "
				 		+ " left outer join tbltallylinkup b on a.strPCode=b.strGroupCode "
				 		+ " where a.strPType='Supp' and a.strClientCode='"+clientCode+"' order by a.strPName  "; 
//			}
	...
	//Save or Update LinkUp
			...
			public ModelAndView funLoadLinkupData(@ModelAttribute("command")clsTallyLinkUpBean objBean,Map<String,Object> model,HttpServletRequest request){
				...
				try{ 
					...
					{
						 sql= " select a.strTaxCode,a.strTaxDesc,a.strTaxOnSP,ifnull(b.strTallyCode,'') "
						 		+ " from tbltaxhd a left outer join tbltallylinkup b on a.strTaxCode=b.strGroupCode "
						 		+ " where  a.strClientCode='"+clientCode+"' order by a.strTaxCode  "; 
					}
					if(objBean.getStrLinkup().equals("SubGroup"))
					{
						sql= " select a.strSGCode,a.strSGName,a.strSGDesc,ifnull(b.strTallyCode,'') "
								+ " from tblsubgroupmaster a "
								+ " left outer join tbltallylinkup b on a.strSGCode=b.strGroupCode  "
								+ " where  a.strClientCode='"+clientCode+"' order by a.strSGCode   ";
					}
