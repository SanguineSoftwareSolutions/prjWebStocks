...
public class clsNonStkMISController {
		    ...
		    if(listGRNHd.size()>0)
		     {
		 			...
		 			if(!tempGRNHd.getStrPONo().equals("") && null!=tempGRNHd.getStrPONo())
		 			{
						...
						for(int j=0;j<strPoCodes.length;j++)
						{
							String sql="select IfNULL(b.strReqCode,'') from tblpurchaseorderdtl a "
			 						...
			 						+ " where a.strPOCode='"+strPoCodes[j]+"' group by b.strReqCode";
				...
				else if(strFlow.equalsIgnoreCase("Direct"))
				{
					...
					//List listTempDtl=objGRNService.funGetDtlList(strGRNCode,clientCode);
					
					String sql="select '',b.strProdCode,d.strProdName,b.dblUnitPrice,sum(b.dblQty),a.strLocCode,c.strLocName "
							+ " from tblgrnhd a ,tblgrndtl b ,tbllocationmaster c,tblproductmaster d "
							+ " where a.strGRNCode=b.strGRNCode and a.strLocCode=c.strLocCode "
							+ " and a.strGRNCode='"+strGRNCode+"' and d.strProdCode=b.strProdCode and d.strNonStockableItem='Yes' "
							+ " and a.strClientCode='"+clientCode+"' and b.strClientCode='"+clientCode+"' "
							+ " and d.strClientCode='"+clientCode+"' group by  b.strProdCode";
						List list=objGlobalFunctionsService.funGetList(sql,"sql");
	...
	public @ResponseBody  clsNonStkProductModel funGetGRNProdData(HttpServletRequest req,HttpServletResponse res)
	{
		...
		String clientCode=req.getSession().getAttribute("clientCode").toString();
		clsNonStkProductModel nonstkmodel= new clsNonStkProductModel();	
		/*String sql="from clsGRNDtlModel a ,clsGRNHdModel b,clsProductMasterModel c,clsLocationMasterModel d"
				+ " where a.strGRNCode=b.strGRNCode and a.strProdCode=c.strProdCode and b.strLocCode=d.strLocCode and "
				+ " a.strGRNCode='"+GRNCode+"' and a.strProdCode='"+prodCode+"' and a.strClientCode='"+clientCode+"' and b.strClientCode='"+clientCode+"'"
				+ " and c.strClientCode='"+clientCode+"' and d.strClientCode='"+clientCode+"' ";*/
		String sql="select a.strProdCode,c.strProdName,d.strLocCode,d.strLocName,sum(a.dblQty) as GRNQty, "
				+ " a.dblUnitPrice from  tblgrndtl a ,tblgrnhd b,tblproductmaster c,tbllocationmaster d "
				+ " where a.strGRNCode=b.strGRNCode and a.strProdCode=c.strProdCode and b.strLocCode=d.strLocCode "
				+ " and  a.strGRNCode='"+GRNCode+"' and a.strProdCode='"+prodCode+"' and a.strClientCode='"+clientCode+"' "
	...
	public String funOpenNonStkMIS(@ModelAttribute("setUpAttribute")@Valid clsNonStkProductBean objBean,BindingResult result,HttpServletRequest req)
	{
		...
		if(!result.hasErrors())
		{
			...
			if(firstObj.getStrReqCode().trim().length()!=0)
			{
				...
				String SqlInsertgrnmisdtl="insert into tblgrnmisdtl(strGRNCode,strMISCode,strClientCode)"
						+ " value('"+strgrnCode+"','"+MISHdModel.getStrMISCode()+"','"+clientCode+"')";
			...
			else
			{
				...
				for(clsNonStkProductModel tempOb:temphdList1)
				{
					if(null!=tempOb.getStrProdCode())
					{
						...
						//map.put(tempOb.getStrToLocCode(), tempOb.getStrToLocName());
					}
				}
				for(String strLocCode:setLocCode)
				{
					...
					String Sql="Insert into tblmishd(strMISCode,intid,dtMISDate,strAgainst,strReqCode,strLocFrom,strLocTo,strNarration,strAuthorise,dtCreatedDate,strUserModified,dtLastModified,strUserCreated,strClientCode)"
						+ "value('"+strMISCode+"',"+lastNo+",'"+dtMISDate+"','"+strAgainst+"','"+strReqCode+"','"+strLocFrom+"','"+strLocTo+"','"+strNarration+"','"+strAuthorise+"','"+dtCreatedDate+"','"+strUserModified+"','"+dtLastModified+"','"+strUserCreated+"','"+clientCode+"')";
					...
					if(res>0)
					{
						...
						for(int i=0;i<List1.size();i++)
						{
							...
							{
								String subSql="insert into tblmisdtl(strMISCode,strProdCode,dblQty,dblUnitPrice,dblTotalPrice,strRemarks,strReqCode,strClientCode)"
										+ " value('"+strMISCode+"','"+Obj1.getStrProdCode()+"',"+Obj1.getDblQty()+","+Obj1.getDblCostRM()+","+Obj1.getDblPrice()+",'"+""+"','"+""+"','"+clientCode+"')";
				...
				String sql="select b.strProdCode, c.strLocName "
						+ " from tblmishd a, tblmisdtl b, tbllocationmaster c "
						+ " where a.strMISCode = b.strMISCode and "
						+ " ("+strMisCodes+") "
						+ " and a.strLocTo = c.strLocCode order by strProdCode ";
				@SuppressWarnings("rawtypes")
				List list=objGlobalFunctionsService.funGetList(sql,"sql");
				if(!list.isEmpty())
				{
					for(int i=0;i<list.size();i++)
					{
						...
						String updateQuery="update tblgrndtl set strRemarks = CONCAT(strRemarks, ' "+locName+"') "
								+ "	where strgrnCode = '"+grnCode+"' and strprodcode = '"+pordCode+"' " ;
				...
				{
					String SqlInsertgrnmisdtl="insert into tblgrnmisdtl(strGRNCode,strMISCode,strClientCode)"
						+ " value('"+grnCode+"','"+tempMISCode[i]+"','"+clientCode+"')";
						objMISService.funInsertNonStkItemDirect(SqlInsertgrnmisdtl);
				}
	...
	{
		String clientCode=req.getSession().getAttribute("clientCode").toString();
		String sql="select a.strGRNCode,Date(a.dtGRNDate) as GRNDate,b.strPName as SupplierName,c.strLocName as LocationName, "
				+ " a.strBillNo,a.strNarration,a.dblTaxAmt,a.dblTotal from tblgrnhd a,tblpartymaster b,tbllocationmaster c "
				+ " where strgrncode IN (select strGRNCode from tblgrndtl a, tblproductmaster b where a.strProdCode = b.strProdCode "
				+ " and b.strNonStockableItem = 'Yes' and a.strClientCode='"+clientCode+"' and b.strClientCode='"+clientCode+"') "
				+ " and strgrncode NOT IN (select strgrncode from tblgrnmisdtl where  strClientCode='"+clientCode+"') "
				+ " and a.strClientCode='"+clientCode+"' and a.strSuppCode=b.strPCode and a.strLocCode=c.strLocCode and a.strLocCode='"+locCode+"'";		
