...
public class clsPOSLinkUpController{
	...
	public ModelAndView funOpenForm(@ModelAttribute("command")clsPOSLinkUpBean objBean,Map<String,Object> model,HttpServletRequest request){
		...
		try
		{
			String sql="select DISTINCT b.strPOSItemCode,b.strPOSItemName,ifnull(b.strWSItemCode,''),ifnull(b.strWSItemName,'') "
					+ "from tblpossalesdtl a,tblposlinkup b "
					+ "where a.strPOSItemCode=b.strPOSItemCode "
...
//Load Master Data On Form
	...
	public @ResponseBody clsPOSLinkUpModel funLoadMasterData(HttpServletRequest request){
		//objGlobal=new clsGlobalFunctions();
		String sql="";
		String clientCode=request.getSession().getAttribute("clientCode").toString();
		String userCode=request.getSession().getAttribute("userCode").toString();
		clsPOSLinkUpBean objBean=new clsPOSLinkUpBean();
		String docCode=request.getParameter("docCode").toString();
		//List listModel=objGlobalFunctionsService.funGetList(sql);
		clsPOSLinkUpModel objPOSLinkUp = new clsPOSLinkUpModel();
		return objPOSLinkUp;
	}*/
		...
		public ModelAndView funAddUpdate(@ModelAttribute("command") @Valid clsPOSLinkUpBean objBean ,BindingResult result,HttpServletRequest req){
			if(!result.hasErrors()){
				...
				for(int cnt=0;cnt<listPOSLinkUp.size();cnt++)
				{
					...
					sql_Update="update tblpossalesdtl set strWSItemCode='"+objModel.getStrWSItemCode()+"' "
						+ "where strPOSItemCode='"+objModel.getStrPOSItemCode()+"' and strClientCode='"+objModel.getStrClientCode()+"' ";
