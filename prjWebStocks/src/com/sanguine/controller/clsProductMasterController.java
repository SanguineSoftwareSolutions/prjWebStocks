...
public class clsProductMasterController
{
	...
	public ModelAndView  funAddUpdate(@ModelAttribute("command") @Valid clsProductMasterBean objBean,BindingResult result,HttpServletRequest req,HttpServletResponse resp,  @RequestParam("prodImage") MultipartFile file) throws IOException
	{
		...
		if(!result.hasErrors())
		{
			...
			{
				String sqlUpdateConversion="update tblproductmaster set strIssueUOM='"+objGeneralModel.getStrUOM()+"'"
					+ ",strRecipeUOM='"+objGeneralModel.getStrUOM()+"' where strProdCode='"+objGeneralModel.getStrProdCode()+"' "
					+ "and strClientCode='"+objGeneralModel.getStrClientCode()+"'";
 			...
 			if(null!=listProdCustMargin)
			{
 				...
 				for(int counter=0;counter<listProdCustMargin.size();counter++)
				{
 					...
 					{
 					//objProductMasterService.funDeleteProdSuppWise(objProdSupp.getStrSuppCode(), objGeneralModel.getStrProdCode(), clientCode);
	...
	public @ResponseBody int funGetProductCountFromTransactions(@RequestParam("prodCode") String prodCode,HttpServletRequest req)
	{
		...
		objGlobal=new clsGlobalFunctions();
		//String clientCode=req.getSession().getAttribute("clientCode").toString();
		String sql="select strProdCode from tblproductmaster where strProdCode='"+prodCode+"' and "
			+ "(strProdCode in (select strProdCode as ProductCode from tblmisdtl) or "
			+ "strProdCode in (select strProdCode as ProductCode from tblgrndtl) or "
			+ "strProdCode in (select strProdCode as ProductCode from tblmaterialreturndtl) or "
			+ "strProdCode in (select strProdCode as ProductCode from tblpurchaseindenddtl) or "
			+ "strProdCode in (select strProdCode as ProductCode from tblpurchaseorderdtl) or "
			+ "strProdCode in (select strProdCode as ProductCode from tblpurchasereturndtl) or "
			+ "strProdCode in (select strProdCode as ProductCode from tblinitialinvdtl) or "
			+ "strProdCode in (select strProdCode as ProductCode from tblstockadjustmentdtl) or "
			+ "strProdCode in (select strProdCode as ProductCode from tblstockpostingdtl) or "
			+ "strProdCode in (select strProdCode as ProductCode from tblstocktransferdtl) or "
			+ "strProdCode in (select strProdCode as ProductCode from tblproductiondtl) or "
			+ "strProdCode in (select strProdCode as ProductCode from tblproductionorderdtl) or "
			+ "strProdCode in (select strProductCode as ProductCode from tblratecontdtl))";
...
//	{
//		String clientCode=req.getSession().getAttribute("clientCode").toString();
//		clsProcessMasterModel objProcessMaster=objProcessService.funGetProcessMaster(processCode, clientCode);
...
//	}
	...
	private clsProductMasterModel funPrepareModelGeneral(clsProductMasterBean objBean,String userCode,String clientCode,MultipartFile file) throws IOException
	{
		...
		objModel.setStrNonStockableItem(objGlobal.funIfNull(objBean.getStrNonStockableItem(),"No","Yes"));
	...
	{
		String clientCode =req.getSession().getAttribute("clientCode").toString();
		boolean count=objGlobalFunctions.funCheckName(Name.trim().toLowerCase(),clientCode,"frmProductMaster");
		return count;
	...
	public ModelAndView funOpenProductListForm(Map<String, Object> model,HttpServletRequest request)
		...
		//return new ModelAndView("frmProductList","command", new clsReportBean());
		return objModelView;
    }
	...
	private void funCallReport(clsReportBean objBean,HttpServletResponse resp,HttpServletRequest req)
	{
		...
		try {
            ...
            {
            	sql=" select p.strProdCode as ProdCode,p.strProdName as ProdName,s.strSGName as SGName,"
                     		+ " p.strUOM as UOM,p.dblCostRM as CostRM,p.dblListPrice as ListPrice,"
                     		+ " l.strlocname as Locname,p.strSpecification as Specification ,p.strBinNo as BinNo "
                     		+ " from tblproductmaster p"
                     		+ " left outer join tblsubgroupmaster s on p.strSGCode=s.strSGCode and s.strClientcode='"+clientCode+"' "
            				+ " left outer join tbllocationmaster l on p.strloccode=l.strloccode and l.strClientcode='"+clientCode+"' "
            				+ " and l.strPropertyCode='"+propertyCode+"' where p.strClientcode='"+clientCode+"' and (p.strLocCode='"+strLocCode+"' or p.strLocCode='') ";
            ...
            else
            {
            	sql="select p.strProdCode as ProdCode,p.strProdName as ProdName,s.strSGName as SGName,"
                 		...
                 		+ " left outer join tblsubgroupmaster s on p.strSGCode=s.strSGCode and s.strClientcode='"+clientCode+"' "
        				+ " left outer join tbllocationmaster l on p.strloccode=l.strloccode and l.strClientcode='"+clientCode+"' "
        				+ " where p.strClientcode='"+clientCode+"' ";
            	if(!objBean.getStrProdType().equalsIgnoreCase("ALL"))
            	{
            		sql+= " and p.strProdType='"+objBean.getStrProdType()+"' ";
            	}
	...
	private ByteArrayInputStream funInputStreamToBytearrayInputStrean(InputStream ins)
	 ...
	 @SuppressWarnings("rawtypes")
		...
		public @ResponseBody List funProdCustomerData(@RequestParam("prodCode") String prodCode,HttpServletRequest req)
		{
			...
			String clientCode=req.getSession().getAttribute("clientCode").toString();
			List<clsProdSuppMasterModel> listProdSupp = new ArrayList<clsProdSuppMasterModel>();
			String sql= " select a.strSuppCode, b.strPName,a.dblStandingOrder,a.dblMargin "
					+ " from tblprodsuppmaster a ,tblpartymaster b  "
					+ " where a.strProdCode='"+prodCode+"' and a.strSuppCode=b.strPCode and b.strPType='cust' and a.strClientCode='"+clientCode+"' "
	 ...
	 @SuppressWarnings("rawtypes")
		...
		public @ResponseBody List funloadSubGroupWiseProduct(@RequestParam("sgCode") String sgCode,HttpServletRequest req)
		{
			...
			try
			{
				//listProd=objProductMasterService.funGetSubGroupWiseProductList(sgCode, clientCode);
