...
public class clsStockController 
{
	...
	//Save or Update.
		...
		public ModelAndView funAddUpdate(@ModelAttribute("command") @Valid clsOpeningStkBean objBean,BindingResult result,HttpServletRequest req)
		{
			...
			if(!result.hasErrors())
			{
				...
				if(null != listOpStkDtl && listOpStkDtl.size() > 0)
		        {
		            ...
		            String updateUnitPriceQuery="update tblproductmaster a join (select dblCostPerUnit as dblCostPerUnit ,strProdCode,strOpStkCode from tblinitialinvdtl) "
		            		+ "b on b.strProdCode = a.strProdCode set a.dblCostRM = b.dblCostPerUnit "
		            		+ " where b.strOpStkCode='"+objHdModel.getStrOpStkCode()+"' and a.strClientCode='"+clientCode+"' and b.dblCostPerUnit > 0 ";
			...
			if(!result.hasErrors())
			{
				...
				if(null != listOpStkDtl && listOpStkDtl.size() > 0)
		        {
					for (clsOpeningStkDtl ob : listOpStkDtl)
		            {
		            	if(null!=ob.getStrProdCode())
		            	{
			            	sql="select a.strOpStkCode from tblinitialinvdtl a,tblinitialinventory b where a.strOpStkCode=b.strOpStkCode "
			            			+ " and a.strProdCode='"+ob.getStrProdCode()+"' and b.strLocCode='"+locCode+"' "
				...
				HttpServletRequest request)
	    {
			...
			if ("2".equalsIgnoreCase(urlHits)) {
				return new ModelAndView("frmOpeningStockSlip_1", "command",
						new clsReportBean());
			} else {
				return new ModelAndView("frmOpeningStockSlip", "command",
						new clsReportBean());
			}
	    ...
	    {        
			objGlobal=new clsGlobalFunctions();
			 Connection con=objGlobal.funGetConnection(req);
				...
				String sql="select a.strOpStkCode,date_format(a.dtCreatedDate, '%d-%m-%Y'),b.strLocName "
						+ " from tblinitialinventory a,tbllocationmaster b "
						+ " where a.strLocCode=b.strLocCode and a.strOpStkCode='"+docCode+"' ";
				List list=objGlobalFunctionsService.funGetList(sql,"sql");	
				...
				String sqlHDQuery="select d.strGName,c.strSGName,a.strProdCode,b.strProdName,a.strUOM,a.dblQty,a.dblCostPerUnit ,(a.dblQty*a.dblCostPerUnit) as Value "
						+ " from tblinitialinvdtl a, tblproductmaster b, tblsubgroupmaster c, tblgroupmaster d  "
						+ " where a.strProdCode=b.strProdCode and b.strSGCode=c.strSGCode and c.strGCode=d.strGCode "
						+ " and a.strOpStkCode='"+docCode+"' and a.strClientCode='"+clientCode+"' and b.strClientCode='"+clientCode+"' "
