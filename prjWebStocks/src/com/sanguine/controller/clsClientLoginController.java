			...
			if(result.hasErrors())
			{
				...
				objMV = new ModelAndView("frmClientLogin","command", new clsClientBean());
					...
					if(objClientBean !=null){
						if(objClientBean.getStrClientCode() !=null && objClientBean.getStrClientCode().trim().length()>0 
									&& objClientBean.getStrPassword() !=null && objClientBean.getStrPassword().trim().length()>0){
