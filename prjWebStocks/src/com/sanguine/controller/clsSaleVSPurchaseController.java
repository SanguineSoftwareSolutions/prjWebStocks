 ...
 private clsPOSDashboardBean FunGetData(String clientCode, String fromDate,String toDate)
  {
	...
	JSONObject	jObj = objGlobalFunctions.funPOSTMethodUrlJosnObjectData("http://localhost:8080/prjSanguineWebService/WebPOSReport/funGetSalePurchaseComparisonDtl",jObjFillter);
	...
	{	
		...
		{				
			...
			else if(jsonObject.get("MonthName").toString().equals("February"))
			{
				 objPOSSaleBean.setStrItemCode("FEB");
				 objPOSSaleBean.setStrItemName("FEB");
			}
			else if(jsonObject.get("MonthName").toString().equals("March"))
			{
				objPOSSaleBean.setStrItemCode("MAR");
			    objPOSSaleBean.setStrItemName("MAR");
			}
			else if(jsonObject.get("MonthName").toString().equals("April"))
			{
				objPOSSaleBean.setStrItemCode("APR");
			    objPOSSaleBean.setStrItemName("APR");
			}
			else if(jsonObject.get("MonthName").toString().equals("May"))
			{
				objPOSSaleBean.setStrItemCode("MAY");
			    objPOSSaleBean.setStrItemName("MAY");
			}
			else if(jsonObject.get("MonthName").toString().equals("June"))
			{
				objPOSSaleBean.setStrItemCode("JUN");
			    objPOSSaleBean.setStrItemName("JUN");
			}
			else if(jsonObject.get("MonthName").toString().equals("July"))
			{
				objPOSSaleBean.setStrItemCode("JUL");
			    objPOSSaleBean.setStrItemName("JUL");
			}
			else if(jsonObject.get("MonthName").toString().equals("August"))
			{
				objPOSSaleBean.setStrItemCode("AUG");
			    objPOSSaleBean.setStrItemName("AUG");
			}
			else if(jsonObject.get("MonthName").toString().equals("September"))
			{
				objPOSSaleBean.setStrItemCode("SEP");
			    objPOSSaleBean.setStrItemName("SEP");
			}
			else if(jsonObject.get("MonthName").toString().equals("October"))
			{
				objPOSSaleBean.setStrItemCode("OCT");
			    objPOSSaleBean.setStrItemName("OCT");
			}
			else if(jsonObject.get("MonthName").toString().equals("November"))
			{
				objPOSSaleBean.setStrItemCode("NOV");
			    objPOSSaleBean.setStrItemName("NOV");
			}
			else if(jsonObject.get("MonthName").toString().equals("December"))
			{
				objPOSSaleBean.setStrItemCode("DEC");
			    objPOSSaleBean.setStrItemName("DEC");
			}
			objPOSSaleBean.setDblAmount((Math.rint(Double.parseDouble(jsonObject.get("TotalPurchaseAmt").toString())))); //purchase Amount
			objPOSSaleBean.setDblSettlementAmt(Math.rint(Double.parseDouble(jsonObject.get("TotalSaleAmt").toString())));   // sale Amount
