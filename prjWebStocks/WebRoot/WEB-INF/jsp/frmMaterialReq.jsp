				    ...
				    success: function(response)
				    {
				    	...
				    	else
				    	{
				    		if(showReqStk=="N")
				    			...
				    			{
				    				if(response.strNonStockableItem=="No")
				    					{
...
// 		 			+ "&ProdCode=" + code;
	...
	{
		var searchUrl="";
		searchUrl=getContextPath()+"/loadLoadStandardProduct.html";
		//alert(searchUrl);
		$.ajax({
		        type: "GET",
		        url: searchUrl,
	...
	<s:form name="matReq" method="POST" action="saveMaterialReq.html?saddr=${urlHits}">
		...
		<table class="transTableMiddle">
			<tr>
				...
				<td colspan="3" id="spProdName"></td>
				<td  colspan=>Stock &nbsp;&nbsp;
				<label id="spStock" class="namelabel"></label>
				<span id="spStockUOM"></span>
				</td>
				
				<td colspan="3">Save Standard List &nbsp; &nbsp; &nbsp;<s:input id="cbSaveStd" value="Y"  path="strSaveSatandard"
					type="checkbox"  /> &nbsp; &nbsp; &nbsp;<input type="button" value="Load Standard" onclick="funLoadStandardProduct()" class="form_button"/>  </td>
				<td><input type="hidden" id="hidNonStkableItem"/></td>
