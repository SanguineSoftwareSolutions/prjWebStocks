...
//				}
	<%}%><%}%>
		...
		function funCallBarCodeProduct(code)
		{
				...
				var searchUrl = getContextPath()+ "/loadProductDataWithTax.html?prodCode=" + code;
...
// 									$("#txtMRP").val((response.dblMRP)/parseFloat(currValue).toFixed(maxQuantityDecimalPlaceLimit));
									...
									$("#txtRate").val((response.dblUnitPrice).toFixed(maxQuantityDecimalPlaceLimit));
// 									$("#txtMRP").val((response.dblMRP).toFixed(maxQuantityDecimalPlaceLimit));
									$("#txtMRP").val((response.dblUnitPrice).toFixed(maxQuantityDecimalPlaceLimit));
...
// 			        			 $("#txtSOCode").val(response.strSOCode);
	...
	function funAddProductDtlRow(objInvDtl) 
	{
		var table = document.getElementById("tblProdDet");
	    ...
	    parseFloat(dblQty).toFixed(maxQuantityDecimalPlaceLimit);
	    var dblRate=objInvDtl.dblUnitPrice;
	    var dblAmount=dblQty*dblRate;
	    var dblMRP=(parseFloat(objInvDtl.dblMRP)* parseFloat(dblUOMConversion)).toFixed(maxQuantityDecimalPlaceLimit);
	    
	    var strCustCode=objInvDtl.strCustCode;
	    var prevInvCode=objInvDtl.prevInvCode;
	    var prevProdrice=objInvDtl.prevUnitPrice;
...
// 		 		 	$("#txtMRP").val((parseFloat(mrp)*parseFloat(conValue)).toFixed(maxQuantityDecimalPlaceLimit));
	 ...
	 function funSetSettlementRow(objSettlement) 
		{
			var table = document.getElementById("tblSettleDet");
							...
							<tr>
								...
								<tr>
									...
									<td  colspan="1"><s:input path="strCustCode" id="txtCustCode"
											ondblclick="funHelp('custMaster')" cssClass="searchTextBox" onkeypress="funGetKeyCode(event,'Customer')" /></td>
