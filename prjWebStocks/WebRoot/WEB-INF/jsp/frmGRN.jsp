   ...
   //Set tab which have Active on form loding
	...
	<script type="text/javascript">
		...
		function funFillTableForAgainst(prodCode,prodName,unitPrice,dblTotalPrice,qtyrecevd,code,
				uom,strExpiry,dblDiscount,strIssueLocation,strIsueLocName,strStkble,strReqCode,unitWeight,strMISCode) 
		{
		...
		function btnAdd_onclick() 
	    {
			...
			if( $("#hidstrStkble").val()=="Yes" && $("#txtIssueLocCode").val()=="")
			{
...
// 			  }
		...
		function funSetProductForPO(code)
		{
			var searchUrl="";
			var POCode=$("#cmbPODoc").val();
			searchUrl=getContextPath()+"/loadProductDataForPOTrans.html?prodCode="+code+"&POCode="+POCode;
			//alert(searchUrl);
			$.ajax({
				type : "GET",
				url : searchUrl,
		...
		function funCalculateIndicatorTax()
		{
		    ...
		    $.ajax({
				type: "GET",
			    url: getContextPath()+"/getTaxDtlForProduct.html?prodCode="+prodCodeForTax+"&taxType=Purchase&transDate="+dteGrn,
			    dataType: "json",
			    success: function(response)
			    {
	...
	/* 	function funCalTaxTotalForBlankIndi()
		...
		**/
		...
		function funSetGRNHd(response) {
			...
			$("#txtDiscAmount").val(parseFloat(response.dblDisAmt).toFixed(maxAmountDecimalPlaceLimit));
			$("#txtNarration").val(response.strNarration);
			$("#txtExtraCharges").val(parseFloat(response.dblExtra).toFixed(maxAmountDecimalPlaceLimit));
			$("#txtLessAmt").val(parseFloat(response.dblLessAmt).toFixed(maxAmountDecimalPlaceLimit));
			...
			funLoadGRNDtl(response.strGRNCode);
			funLoadGRNTaxDtl(response.strGRNCode);
		//	funCalTaxTotal();
		}

		
		//Get GrnDtl data
		function funLoadGRNDtl(code) {
			var searchUrl = "";
		...
		//Get grn taxDtl data
		function funLoadGRNTaxDtl(code) 
		{
			var searchUrl = "";
			...
			$.ajax({
				...
				success : function(response) 
				{
					...
					$.each(response, function(i,item)
		            {
						funAddTaxRow1(response[i].strTaxCode,response[i].strTaxDesc,response[i].strTaxableAmt,response[i].strTaxAmt);
		            });
				},
				error : function(jqXHR, exception) {
					if (jqXHR.status === 0) {
		...
		function funAddProductForGRN(prodCode, prodName, uom, unitPrice,wtunit, qtyrecevd, qtyrecb, dcWt, dcQty, rejected, poWt
			...
			rework = parseFloat(rework).toFixed(maxQuantityDecimalPlaceLimit);
			packforward = parseFloat(packforward).toFixed(maxQuantityDecimalPlaceLimit);
			disc = parseFloat(disc).toFixed(maxAmountDecimalPlaceLimit);
			...
			$.ajax({
					...
					success : function(response) {
						...
						if ('Invalid Code' == response.strProdCode) {
							...
							if(response.strNonStockableItem=="Yes")
							{
								$("#lblIssueLocation").css('visibility', 'visible');
								$("#lblIssueLocName").css('visibility', 'visible');
								$("#txtIssueLocCode").css('visibility', 'visible');
						...
						success : function(response) {
							$.each(response, function(i, item) {
								...
								funFillTableForAgainst(response[i].strProdCode,response[i].strProdName,response[i].dblRate
									...
									,response[i].strIsueLocName,response[i].strStkble,response[i].strReqCode
						...
						<div id="tab1" class="tab_content" style="height: 550px">
							<table class="transTable">			
							    ...
							    <tr>
							        ...
							        <td><s:input id="txtSuppCode" required="required" path="strSuppCode"  ondblclick="funHelp('suppcode')" cssClass="searchTextBox"/></td>
							        <td colspan="2"><s:label id="txtSuppName" path="strSuppName" /></td>
							    ...
							    <tr>
							        ...
							        <td colspan="7"><s:input id="txtInwRefNo" path="strRefNo" cssClass="BoxW116px"/></td>
								</tr>
							 ...
							 <tr>
								...
								<tr>
									...
									<td><input id="txtProdCode" ondblclick="funOpenHelp()" onblur="funAlert(this)" class="searchTextBox"></input></td>
									<td colspan="2"><label id="lblProdName"></label></td>
