...
<script type="text/javascript">
	...
	**/
		...
		function funCalculateIndicatorTax()
		{
		    ...
		    gurl=getContextPath()+"/getTaxDtlForProduct.html?prodCode="+prodCodeForTax+"&taxType=Purchase&transDate="+dteOP,
			//alert(gurl);
		    	$.ajax({
			   		type: "GET",
			        ...
			        success: function(response)
			        {
			        	$.each(response, function(i,item)
					    {
			        		...
			        		else
					    	{
					        	...
					        	//alert(taxCode,taxDesc,taxableAmt,taxAmt);
		...
		function funCalTaxTotal()
		{
			...
			var finalAmt=parseFloat(subTotal)+parseFloat(totalTaxAmt)+parseFloat(extCharge)-parseFloat(disAmt);
		...
		/* function funOnChange()
		{
			...
			else if($("#cmbAgainst").val()=="Purchase Indent")
			{
		...
		function funOpenPIforPO(transactionName)
		{
			...
			//	var retval=window.showModalDialog("frmPIforPO.html","","dialogHeight:600px;dialogWidth:600px;dialogLeft:400px;")
				var retval=window.open("frmPIforPO.html","","dialogHeight:600px;dialogWidth:600px;dialogLeft:400px;")
		       //alert(retval);
								...
								if (retval.returnValue != null)
								{
									strVal=retval.returnValue.split("#")
									 ...
									 var PICodes=strVal[0].split(",");
										...
										{
						 						html += '<option value="' + PICodes[cnt] + '" >' + PICodes[cnt]
						 								+ '</option>';
						 					//html += '</option>';
...
// 		    }		    
		    ...
		    if($("#txtSuppCode").val()=="")
			{
		    	strSuppCodeTag="<input type=\"text\"  name=\"listPODtlModel["+(rowCount)+"].strSuppCode\" class=\"Box suppCodeCellValue\"  required=\"required\" size=\"5%\" id=\"txtSuppCode."+(rowCount)+"\" value='"+strSuppCode+"'/>";
		    	strSCodeTag="<input id=btnSup"+rowCount+" type=button  onclick=funHelp1("+(rowCount)+",'suppcode1') value=...>";
		    	strSNameTag="<input name=\"listPODtlModel["+(rowCount)+"].strSuppName\" readonly=\"readonly\" class=\"Box\" class=\"inputText-Auto\" id=\"txtSuppName."+(rowCount)+"\" value='"+SupplierName+"'/>";
			}
		    else
		    {		    	
		    	strSuppCodeTag="<input name=\"listPODtlModel["+(rowCount)+"].strSuppCode\" readonly=\"readonly\" class=\"Box\" size=\"5%\" id=\"txtSuppCode."+(rowCount)+"\" value='"+strSuppCode+"'/>";
		    	strSCodeTag="";
		    	strSNameTag="<input name=\"listPODtlModel["+(rowCount)+"].strSuppName\" readonly=\"readonly\" class=\"Box inputText-Auto\" id=\"txtSuppName."+(rowCount)+"\" value='"+SupplierName+"'/>";
		    }
							...
							<table class="transTable">
								...
								<tr>
									...
									<td><s:input path="strSuppCode" id="txtSuppCode"
											ondblclick="funHelp('suppcode')" cssClass="searchTextBox" /></td>
								...
								<tr>
									...
									<td colspan="4" align="left"><s:select path="strCurrency"
											id="cmbCurrency" cssClass="BoxW62px">
...
<!-- 									<td><input id="txtDiscount" type="text" class="decimal-places-amt numberField" value="0" class="BoxW116px" /></td> -->
							...
							<table id="tbl2" class="transTable">
								<tr>
									...
									<td><s:input type="text" id="txtYourRef" path="strYourRef"
											cssClass="simpleTextBox" /></td>
